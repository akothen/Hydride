...
class RoseRegion:
  def __init__(self, Children : list, Parent):
    ...
    if Parent is not None:
      assert isinstance(Parent, RoseForLoop) \
          or isinstance(Parent, RoseFunction)
    self.Parent = Parent

    # Children do not have to be instances of regions
    for Child in Children:
      assert isinstance(Child, RoseForLoop) \
          or isinstance(Child, RoseFunction) \
  ...
  def addBlock(self, Block):
    ...
    assert isinstance(self, RoseForLoop) \
        or isinstance(self, RoseFunction)
  ...
  def addOperation(self, Op):
    ...
    assert isinstance(self, RoseBlock)

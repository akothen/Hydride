...
def print_sliced_update(slices):
    return "(concat {})".format(slices)


def print_concat_update(lhs, rhs):
    return "(concat {} {})".format(lhs, rhs)


def print_update(lhs, rhs):
    return "(define {} {}) {}".format(lhs, rhs, lhs)


def print_new_bv_define(name, sz):
    return "(define {} (bv 0 {}))".format(name, sz)


def print_bv_define(name, val, sz):
    return "(define {} (bv {} {}))".format(name, val, sz)


def print_bv_expand(hb, lb):
    return "(concat (bv 0 {}) {})".format(hb, lb)


def print_const_bv(val, sz):
...
def print_for(iterator, begin, end, incr, body):
    return "(apply concat (for/list ([{} (reverse (range {} {} {}))]) {}))".format(
...
def print_case(cond, stmt):
    return "[{} {}]".format(cond, stmt)


def print_match(val, cases):
    return "(cond {})".format(cases)
...
def print_provide(func, args):
    return "(provide {} {})".format(func, args)

...
skips = [
    "gf2p8affine", "rotl64", "rotr64", "_mm_cvtsi128_si64x", "_mm_cvtsd_si64x",
    "_mm_cvttsd_si64x", "_mm_cvtsi64x_sd", "_mm_cvtsi64x_si128"
]

todo = {}
dont = True
for i in range(1, len(sys.argv)):
    todo[sys.argv[i].lstrip().rstrip()] = True
...
for intrin in intrins:
    ...
    if do_skip:
        continue
    if len(todo) != 0 and spec.intrin not in todo:
        continue
    if spec.intrin == "_mm512_kunpackw":
        dont = False
        pass
    if dont:
        pass
    ...
    #    print("params -> \n{}\n".format(spec.params))
    #    print("rettype -> \n{}\n".format(spec.rettype))
    intrin = test_gen2.Intrin(spec.rettype, spec.intrin, spec.params)
    TG.add_intrin(intrin, cvt.cvt_all())

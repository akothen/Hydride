...
(define (expr vars #:depth k)
  ...
  (cond
    [(choose* #t #f)
     (apply choose* vars)]
    [(choose* #t #f)
       (vec-load (expr vars #:depth (- k 1)) 64 0 4 8)
    ]
    [(choose* #t #f)
       (vec-load (expr vars #:depth (- k 1)) 64 4 4 8)
    ]
    [(choose* #t #f)
       (vec-load (expr vars #:depth (- k 1)) 192 0 6 8)
    ]
    [(choose* #t #f)
       (vec-load (expr vars #:depth (- k 1)) 192 6 6 8)
    ]
    [(choose* #t #f)
       (vec-load (expr vars #:depth (- k 1)) 192 12 6 8)
    ]
    [(choose* #t #f)
       (vec-load (expr vars #:depth (- k 1)) 192 18 6 8)
    ]
    [(choose* #t #f)
     (vec-shuffle-special
                    (expr vars #:depth (- k 1))

...
(define grammar-fns 
  (list
    (lambda (i) (operations (list (reg 0)) #:depth i  )  )
    (lambda (i) (operations (list (reg 0)) #:depth i  )  )
    (lambda (i) (operations (list (reg 0)) #:depth i  )  )
    (lambda (i) (operations (list (reg 0) (reg 1)) #:depth i  )  )
    (lambda (i) (operations (list (reg 0) (reg 1)) #:depth i  )  )
    (lambda (i) (operations (list (reg 0)) #:depth i  )  )
    ))
...
(for/list ([i (range (length spec_exprs))])
          ...
          (define-values (satisfiable? materialize elap) 
                         (synthesize-sol-with-depth 1 depth-limit sub_expr test_grammar-fn bw-list #f cost) )

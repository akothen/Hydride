...
class TypedSimpleGrammarGenerator:
  ...
  def emit_grammar_clause(self, dsl_inst, grammar_args , vars_name = "vars",
      ...
      for ga in grammar_args:
        if ga.is_hole:
          grammar_clause.append(self.emit_grammar_hole(vars_name, depth_name, grammar_name) \
                                + " " + ga.get_rkt_comment())
        else:
  ...
  def emit_other_grammar_reference(self, other_grammar, last_clause = False, 
                                   vars_name = "vars", depth_name = "k"):
    command = []
    if last_clause:
      command = ["\t[else"] + [self.emit_grammar_hole(\
                  vars_name, depth_name, other_grammar, decrement = False)] +["]"]
    else:
      command = ["\t[(choose* #t #f)"] + [self.emit_grammar_hole(\
                  vars_name, depth_name, other_grammar, decrement = False)] +["]"]
    return "\n\t".join(command)
  ...
  def emit_grammar_layer(self, layer_name = "grammar_layer", layer_dsl_insts = [],
                          ...
                          other_grammars = [],
                          lit_holes = []):
    assert len(layer_dsl_insts) == len(layer_dsl_args_list), \
            "Each grammar rule should have an argument context"
    ...
    for i in range(0, len(layer_dsl_insts)):
      clauses.append(self.emit_grammar_clause(layer_dsl_insts[i], layer_dsl_args_list[i],
                                              ...
                                              last_clause = (i == len(layer_dsl_insts) -1) \
                                              and len(other_grammars) == 0 and len(lit_holes) == 0))
    for idx, og in enumerate(other_grammars):
      clauses.append(self.emit_other_grammar_reference(og, last_clause = (len(lit_holes) == 0) \
                                                      and (idx == len(other_grammars) -1), 
                                                       vars_name = vars_name,
  ...
  def emit_typed_grammar_clause(self, dsl_inst, ctx , vars_name = "vars",
    ...
    for ga in grammar_args:
      if ga.is_hole:
        ...
        grammar_clause.append(self.emit_grammar_hole(\
                vars_name, depth_name, grammar_name) + " " + ga.get_rkt_comment())
      else:
        grammar_clause.append(ga.get_dsl_value() +"\t\t\t\t"+ga.get_rkt_comment())
    combined = "\n\t\t".join(grammar_clause)
  ...
  def emit_typed_grammar_layer(self, layer_name = "grammar_layer", 
                                layer_dsl_insts = [], layer_dsl_args_list = [],
  ...
  def init_typed_context(self, output_size):
    if output_size not in self.typed_contexts:
      self.typed_contexts[output_size] = \
        { "ops": [], "ctx": [] , "name": self.get_layer_name(output_size), "lits" : [], "regs":  [] }
  ...
  def emit_grammar(self,
      ...
      grammars = [size_to_expr_map[bv_sizes] for bv_sizes in self.typed_contexts \
                                              if bv_sizes in self.reachable_sizes]
      prefix = ";; " + "="*80 + "\n"
      ...
      grammar_tree = "(define-grammar (" + simple_grammar_name + " " \
                      + vars_name+")\n" + " \n\n".join(grammars) + ")"

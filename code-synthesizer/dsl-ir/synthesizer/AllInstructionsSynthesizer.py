...
class AllInstructionsSynthesizer(SynthesizerBase):
    ...
    def __init__(self, spec=None, dsl_operators=[],
        ...
        if target == "x86":
            dsl_operators = [d for d in dsl_operators if "mov" not in d.name]
        ...
        # EXPERIMENT
        self.contexts_per_dsl_inst = 50
        self.step = step
        debug("Using All Instruction")
    ...
    def emit_synthesis_grammar(self, main_grammar_name="synth_grammar", use_lit_holes=USE_LIT_HOLES, include_manual_swizzles= False):
        ...
        assert (self.step == 0), "Repeating previous step"
        ...
        for dsl_inst in self.dsl_operators:
            ...
            if True:
                ...
                min_contexts = min(len(dsl_inst.contexts), self.contexts_per_dsl_inst)

                operator_contexts = dsl_inst.contexts[:min_contexts]
        ...
        if self.N is not None:
            operation_dsl_insts  = operation_dsl_insts[:self.N]
            operation_dsl_args_list = operation_dsl_args_list[:self.N]

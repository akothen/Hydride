...
class VisitorDef:
    ...
    def emit_default_def(self, struct_definer ,  visitor_name = "hydride:visitor"):
        ...
        for structs in default_structs:
            defaults.append(self.emit_visitor_def(structs, struct_definer, visitor_name = visitor_name )[1:])
    ...
    def emit_visit_expr(self, expr, visitor_name = "hydride:visitor"):
        ...
        definition = "(define {} ({} {} fn))".format(folded_name, visitor_name, expr)
    ...
    def emit_visitor_def(self, dsl_inst, struct_definer, visitor_name = "hydride:visitor"):
        ...
        for idx,arg in enumerate(sample_ctx.context_args):
            ...
            if isBitVectorType(arg):
                (folded_name, definition) = self.emit_visit_expr(arg.name, visitor_name = visitor_name)
    ...
    def emit_visitor(self, dsl_inst_ls, struct_definer, visitor_name = "hydride:visitor"):
        ...
        interpret_clauses = self.emit_default_def(struct_definer, visitor_name = visitor_name)
        interpret_clauses += [self.emit_visitor_def(dsl_inst, struct_definer, visitor_name = visitor_name) for dsl_inst in dsl_inst_ls]

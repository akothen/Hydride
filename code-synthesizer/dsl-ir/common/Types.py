...
class BitVector(OperandType):
  ...
  def __hash__(self):
      return self.name + str(self.size) \
        + str(hash(OperandType.OperandTypeEnum.BitVector))
  ...
  def __eq__(self, other):
      ...
      if isinstance(other, LaneSize):
          return (other.value == self.value) \
            and (other.input_precision == self.input_precision) \
            and (other.output_precision == self.output_precision)
      return False
...
class Precision(OperandType):
  ...
  def print_operand(self, prefix = ""):
      print("{} {}\t| Precision {} bits , Is Input  = {}, Is Output = {}"\
        .format(prefix, self.name, self.value, self.input_precision, self.output_precision))
...
class IndexExpr(OperandType):
  ...
  def get_dsl_value(self):
      ...
      assert self.expr_type == IndexExprTypeEnum.Mul
...
class Reg(OperandType):
  ...
  def get_halide_dsl_value(self):
      ...
      return "(buffer-index  " + str(self.index) + " " + type_str + " " \
              + str(self.size) + ") ; < {} x i{}> {}".format(self.size // self.precision, \
                self.precision, self.signed)

  def get_dsl_value(self):
      return "(reg (bv " + str(self.index) \
        + " (bitvector 8))) ; < {} x i{}> {}".format(self.size // self.precision, \
          self.precision, self.signed)
...
def isBitVectorType(arg):
  return isinstance(arg, BitVector) \
      or isinstance(arg, ConstBitVector) \
      or isinstance(arg, BoundedBitVector)

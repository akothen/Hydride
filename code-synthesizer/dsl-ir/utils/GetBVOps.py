...
class GetBVOps:
    ...
    def emit_default_def(self, struct_definer):
        ...
        for structs in default_structs:
            defaults.append(self.emit_get_bv_ops_def(
        ...
        for idx, arg in enumerate(ctx.context_args):
            ...
            if isinstance(arg, BitVector):
                recursive_calls.append("({} {})".format(
                    self.get_ops_name, get_arg(idx).name))
    ...
    def emit_get_bv_ops_def(self, dsl_inst, struct_definer, use_conditional=True):
        ...
        for ctx in dsl_inst.contexts:
            cond_clauses.append(self.emit_ctx_ops_clause(
                ctx, dsl_inst.get_sample_context(), use_conditional=use_conditional))

        cond_clauses.append(
            "[else (error \"Unable to get ops  for {}\")]".format(dsl_inst.name))
        cond_expr = "(cond \n\t\t"+"\n\t\t".join(cond_clauses) + "\n)\n"
    ...
    def emit_get_bv_ops(self, dsl_inst_list, struct_definer):
        ...
        ops_clauses += [self.emit_get_bv_ops_def(dsl_inst, struct_definer)

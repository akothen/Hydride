...
def create_random_expression(intrinsics, required_output_precision=None,
    ...
    if depth == 0:
        ...
        reg_ = Reg(arg_start_idx, required_output_precision,
    ...
    supported_ctxs_idxs = get_supported_insts_supports_config(
        intrinsic, required_output_precision=required_output_precision, required_output_size=required_output_size)
    ...
    if sample_ctx.is_bounded:
        ...
        if required_output_precision == None:
            output_precs = [int(prec)
                            for prec in sample_ctx.out_bound_map.keys()]
            sample_ctx.specialize_context_bounded(
                random.sample(output_precs, 1)[0])
        else:
    ...
    if depth == 1:
        ...
        for idx, arg in enumerate(sample_ctx.context_args):
            if isinstance(arg, BitVector):
                ...
                if arg.size in [8, 16, 32, 64]:  # Scalar values
                    input_prec = arg.size
                (leaf_expr, arg_start_idx) = create_random_expression(intrinsics, required_output_size=arg.size,
                                                                      required_output_precision=input_prec, depth=depth - 1, arg_start_idx=arg_start_idx)

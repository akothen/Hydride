...
class GetOutPrecDef:
    ...
    def emit_default_def(self, struct_definer, env_name="env"):
        ...
        defaults.append(
            "[(reg id) (bvlength (vector-ref-bv {} id))] ;; FIX-ME NOTE: DO NOT USE THIS METHOD FOR PREC".format(env_name))

        defaults.append("[(lit v) (bvlength v)]")

        defaults.append("[(nop v1) ({} v1 {})]".format(
            self.get_prec_name, env_name))
    ...
    def emit_get_prec_def(self, dsl_inst, struct_definer,
        ...
        for ctx in dsl_inst.contexts:
            cond_clauses.append(self.emit_ctx_prec_clause(
                ctx, dsl_inst.get_sample_context()))

        cond_clauses.append(
            "[else (error \"Unable to infer prec for {}\")]".format(dsl_inst.name))
    ...
    def emit_get_prec(self, dsl_inst_list, struct_definer, env_name="env"):
        ...
        prec_clauses += [self.emit_get_prec_def(

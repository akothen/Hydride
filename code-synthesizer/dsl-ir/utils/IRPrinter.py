...
class IRPrinter:
    ...
    def __init__(self, printer_name = "hydride-printer", get_length_name = "get-length", get_prec_name = "get-prec"):
    ...
    def emit_default_def(self, struct_definer ):
        ...
        for structs in default_structs:
            if structs.name == "llvm_shuffle_vectors":
                continue
    ...
    def emit_print_clause(self, dsl_inst, struct_definer):
        return "[{} \n\t(string-append \"\\n\" \n\t{}\n\t{} \"\\n\")]".format(struct_definer.emit_dsl_struct_use(dsl_inst),
                                                   ...
                                                   self.emit_output_type_str(dsl_inst, struct_definer, get_len_name = self.get_length_name, get_prec_name = self.get_prec_name))



    def emit_bounded_base_print_expr(self, dsl_inst):
        sample_ctx = dsl_inst.get_sample_context()
        assert sample_ctx.is_bounded, "Bounded base printer should only be for bounded ctx"

        variable_idx = sample_ctx.in_precision_index[0][1]

        ir_args = dsl_inst.get_sample_context().context_args

        input_prec_arg = None

        define_prec_str = "(define prec-arg (cond\n"


        for ip in sample_ctx.in_bound_map:
            define_prec_str += "[(equal? {} (bitvector->integer {}))  {}]\n".format(ip, ir_args[variable_idx].name, ip)


        define_prec_str += "))\n"


        input_prec_arg = Integer("prec-arg", value = 16)




        str_expr = define_prec_str
    ...
    def emit_base_print_expr(self, dsl_inst):
        ...
        if dsl_inst.has_bounded_behavior():
            return self.emit_bounded_base_print_expr(dsl_inst)
        ...
        for ir_arg in ir_args:
            ...
            if input_prec_arg == None :
                ...
                lit_typeinfo = "(begin (define-values (num_elem arg_prec) (cond [(< (/ ({} {} (vector 0)) {}) 1)  (values  (/ ({} {} (vector 0)) ({} {} (vector 0))) (/ ({} {} (vector 0))) )] [else (values (/ ({} {} (vector 0)) {}) {})]))".format(self.get_length_name, ir_arg.name, input_prec_arg.name, self.get_length_name ,ir_arg.name, self.get_prec_name, ir_arg.name, self.get_prec_name , ir_arg.name, self.get_length_name, ir_arg.name, input_prec_arg.name, input_prec_arg.name)
                lit_typeinfo += "(string-append ({} {}) \" ; \" \"<\" {} \" x i\" {} \">\" \"\\n\" )".format(self.printer_name, ir_arg.name, "(~s num_elem)", "(~s " + "arg_prec" +")")
                lit_typeinfo += ")"

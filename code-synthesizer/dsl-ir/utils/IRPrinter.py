...
class IRPrinter:
    ...
    def emit_default_def(self, struct_definer):
        ...
        load_dsl_use = struct_definer.emit_dsl_struct_use(
            dummy_vector_load_dsl)
        load_args = dummy_vector_load_dsl.get_sample_context().context_args
        load_len_str = "(string-append \"({} \"".format(
            dummy_vector_load_dsl.get_dsl_name())
        load_len_expr = "(* {} {})".format(
            load_args[3].name, load_args[4].name)
    ...
    def emit_bounded_base_print_expr(self, dsl_inst):
        ...
        for ip in sample_ctx.in_bound_map:
            define_prec_str += "[(equal? {} (bitvector->integer {}))  {}]\n".format(
                ip, ir_args[variable_idx].name, ip)
    ...
    def emit_base_print_expr(self, dsl_inst):
        ...
        if input_prec_arg == None:
            ...
            all_same = all([(ip == ip_0) and (ip != None)
                           for ip in input_precisions])
    ...
    def emit_output_type_str(self, dsl_inst, struct_definer, get_len_name="get-length", get_prec_name="get-prec", prog_name="prog"):
        ...
        num_elem_str = "(~s (/ ({} {} (vector 0)) {}) )".format(get_len_name,
                                                                prog_name, get_prec_expr)
    ...
    def emit_dsl_printer(self, dsl_inst_list, struct_definer, prog_name="prog"):
        ...
        print_clauses += [self.emit_print_clause(dsl_inst, struct_definer)

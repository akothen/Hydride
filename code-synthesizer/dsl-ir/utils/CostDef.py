...
class CostDef:
    ...
    def emit_default_def(self, struct_definer, cost_name="hydride:cost", use_label=False):
        ...
        for struct in default_structs:
            defaults.append(self.emit_dsl_cost_def(
                struct, struct_definer, use_label=False, cost_name=cost_name)[1])
    ...
    def emit_dsl_cost_def(self, dsl_inst, struct_definer, use_label=True, cost_name="hydride:cost"):
        ...
        cost_cmd_list = [struct_definer.emit_dsl_struct_use(
            dsl_inst)] + [cost_clause]
    ...
    def emit_cost_model(self, dsl_list, struct_definer, cost_name="hydride:cost", use_label=True):
        default_costs = self.emit_default_def(
            struct_definer, cost_name=cost_name)

        dsl_costs = [self.emit_dsl_cost_def(
            dsl_inst, struct_definer, cost_name=cost_name, use_label=use_label) for dsl_inst in dsl_list]

        cost_defs = [dsl_cost[0] for dsl_cost in dsl_costs] + ["\n"]

        cost_clauses = default_costs + \
            [dsl_cost[1] for dsl_cost in dsl_costs] + [self.emit_fallback_def()]

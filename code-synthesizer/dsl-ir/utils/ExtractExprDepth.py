...
class ExtractExprDepth:
    ...
    def emit_default_def(self, struct_definer):
        ...
        for structs in default_structs:
            defaults.append(self.emit_extract_def(
    ...
    def emit_ctx_extract_clause(self, dsl_inst, ctx, sample_ctx, use_conditional=True):
        ...
        for idx, arg in enumerate(ctx.context_args):
            ...
            if isinstance(arg, BitVector):
                ...
                if len(recursive_calls) == 0:
                    ...
                    prev_num_consumed_name = "num-consumed-{}".format(
                        len(recursive_calls)-1)

                recursive_calls.append("({} {} {} (- depth 1))".format(
                    self.extract_name, get_arg(idx).name, prev_num_consumed_name))
        ...
        if len(recursive_calls) != 0:
            latest_num_consumed_name = "num-consumed-{}".format(
                len(recursive_calls)-1)
    ...
    def emit_extract_def(self, dsl_inst, struct_definer, use_conditional=True):
        ...
        for ctx in dsl_inst.contexts:
            cond_clauses.append(self.emit_ctx_extract_clause(
                dsl_inst, ctx, dsl_inst.get_sample_context(), use_conditional=use_conditional))

        cond_clauses.append(
            "[else (error \"Unable to extract  for {}\")]".format(dsl_inst.name))
        cond_expr = "(cond \n\t\t"+"\n\t\t".join(cond_clauses) + "\n)\n"
    ...
    def emit_extract(self, dsl_inst_list, struct_definer):
        ...
        ops_clauses += [self.emit_extract_def(dsl_inst, struct_definer)

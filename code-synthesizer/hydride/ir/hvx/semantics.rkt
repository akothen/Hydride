#lang rosette
(require rosette/lib/synthax)
(require rosette/lib/angelic)
(require racket/pretty)
(require data/bit-vector)
(require rosette/lib/destruct)
(require rosette/solver/smt/boolector)

(require hydride/utils/bvops)
(require hydride/utils/misc)


(provide (all-defined-out))


;; ================================================================================
;;                                DSL Semantics
;; ================================================================================

(define (hexagon_V6_vrmpybv_acc_128B  Vx Vu Vv )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 8))])
                            (define %1 (+  i.new  31))
                            (define %2 (extract  %1 i.new Vx))
                            (define %5 (+  iterator.0.new  i.new))
                            (define %6 (+  %5  7))
                            (define %7 (extract  %6 %5 Vu))
                            (define %12 (extract  %6 %5 Vv))
                            (define %13 (sign-extend  %7 (bitvector 16)))
                            (define %14 (sign-extend  %12 (bitvector 16)))
                            (define %15 (bvmul  %13  %14))
                            (define %16 (sign-extend  %15 (bitvector 32)))
                            (define %17 (bvadd  %2  %16))
                            %17
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vmpybusv_128B  Vv.1 Vu.1 %outerlanesize0.1 %innerlanesize0.1 %elemsize0.1 %arg0.1 %arg1.1 )
  (define Vdd.1
    (apply
      concat
      (for/list ([i.new.1 (reverse (range 0 2048 %outerlanesize0.1))])
                (apply
                  concat
                  (for/list ([iterator.0.new.1 (reverse (range 0 %innerlanesize0.1 %elemsize0.1))])
                            (define %1.new0.1 (/  i.new.1  2))
                            (define %2.1 (+  iterator.0.new.1  %1.new0.1))
                            (define %lastidx1.1 (-  %elemsize0.1  1))
                            (define %3.1 (+  %2.1  %lastidx1.1))
                            (define %4.1 (extract  %3.1 %2.1 Vu.1))
                            (define %9.1 (extract  %3.1 %2.1 Vv.1))
                            (define %10.1 (zero-extend  %4.1 (bitvector %arg0.1)))
                            (define %11.1 (sign-extend  %9.1 (bitvector %arg0.1)))
                            (define %12.1 (bvmul  %10.1  %11.1))
                            %12.1
                            )
                  )
                )
      )
    )
  Vdd.1
  )

(define (hexagon_V6_vrmpyubv_acc_128B  Vx Vu Vv )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 8))])
                            (define %1 (+  i.new  31))
                            (define %2 (extract  %1 i.new Vx))
                            (define %5 (+  iterator.0.new  i.new))
                            (define %6 (+  %5  7))
                            (define %7 (extract  %6 %5 Vu))
                            (define %12 (extract  %6 %5 Vv))
                            (define %13 (zero-extend  %7 (bitvector 16)))
                            (define %14 (zero-extend  %12 (bitvector 16)))
                            (define %15 (bvmul  %13  %14))
                            (define %16 (zero-extend  %15 (bitvector 32)))
                            (define %17 (bvadd  %2  %16))
                            %17
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vmpyowh_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vu))
                            (define %5 (+  16  %0.new0))
                            (define %6 (+  %5  15))
                            (define %7 (extract  %6 %5 Vv))
                            (define %9 (sign-extend  %2 (bitvector 64)))
                            (define %10 (sign-extend  %7 (bitvector 64)))
                            (define %11 (bvmul  %9  %10))
                            (define %12 (bvlshr  %11  (bv 14 64)))
                            (define %13 (bvadd  %12  (bv 1 64)))
                            (define %14 (bvlshr  %13  (bv 1 64)))
                            (define %15 (bvssat %14 64 32 ))
                            %15
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_lvsplatw_128B  Rt )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 32))])
                            Rt
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vsubbnq_128B  Vx.norm Vu.norm QvV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define i.new.div.norm (/  i.new.norm  %arg2.norm))
                            (define %0.norm (extract  i.new.div.norm i.new.div.norm QvV.norm))
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %2.norm (+  i.new.norm  %lastidx0.norm))
                            (define %3.norm (extract  %2.norm i.new.norm Vx.norm))
                            (define %9.norm (extract  %2.norm i.new.norm Vu.norm))
                            (define %10.norm (bvsub  %3.norm  %9.norm))
                            (define %cond0.norm (bveq  %0.norm  (bv 1 1)))
                            (define %11.norm (if (equal? %cond0.norm #t) %3.norm %10.norm))
                            %11.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_veqh_xor_128B  %arg0.norm Vv.norm Vu.norm %arg1.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %5.new0.norm (+  i.norm  %lastidx0.norm))
                            (define %6.norm (extract  %5.new0.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bveq  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvxor  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vrmpyub_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 8))])
                            (define %1 (+  i.new  31))
                            (define %2 (extract  %1 i.new Vx))
                            (define %5 (+  iterator.0.new  i.new))
                            (define %6 (+  %5  7))
                            (define %7 (extract  %6 %5 Vu))
                            (define %9 (+  iterator.0.new  7))
                            (define %10 (extract  %9 iterator.0.new Rt))
                            (define %11 (zero-extend  %7 (bitvector 16)))
                            (define %12 (zero-extend  %10 (bitvector 16)))
                            (define %13 (bvmul  %11  %12))
                            (define %14 (zero-extend  %13 (bitvector 32)))
                            (define %15 (bvadd  %2  %14))
                            %15
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vaddhsat_128B  Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvaddnsw  %2  %5 %elemsize0))
                            %6
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vaddhw_acc_128B  Vxx Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg1))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %arg0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %8 (+  %7  %lastidx1))
                            (define %9 (extract  %8 %7 Vu))
                            (define %14 (extract  %8 %7 Vv))
                            (define %15 (bvadd  %9  %14))
                            (define %16 (sign-extend  %15 (bitvector %arg0)))
                            (define %17 (bvadd  %4  %16))
                            %17
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vzb_128B  Vu %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (zero-extend  %4 (bitvector %arg0)))
                            %9
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmaxw_128B  Vu.norm Vv.norm %elemsize0.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %1.norm (+  i.new.norm  %lastidx0.norm))
                            (define %2.norm (extract  %1.norm i.new.norm Vu.norm))
                            (define %5.norm (extract  %1.norm i.new.norm Vv.norm))
                            (define %6.norm (bvsgt  %2.norm  %5.norm))
                            (define %13.norm (if (equal? %6.norm #t) %2.norm %5.norm))
                            %13.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_pred_or_n_128B  QsV QtV )
  (define QdV
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i (reverse (range 0 128 1))])
                            (define %0 (extract  i i QsV))
                            (define %1 (extract  i i QtV))
                            (define %2 (bvnot  %1))
                            (define %3 (bvor  %0  %2))
                            %3
                            )
                  )
                )
      )
    )
  QdV
  )

(define (hexagon_V6_veqw_and_128B  %arg0.norm %arg1.norm Vu.norm Vv.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %5.new0.norm (+  i.norm  %lastidx0.norm))
                            (define %6.norm (extract  %5.new0.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bveq  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvand  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vpackhub_sat_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.new (reverse (range 0 512 8))])
                              (define %9.new0 (*  %iterator1.new  2))
                              (define %10 (+  %9.new0  15))
                              (define %11 (extract  %10 %9.new0 Vu))
                              (define %12 (bvusat %11 16 8 ))
                              %12
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new (reverse (range 0 512 8))])
                              (define %0.new0 (*  %iterator0.new  2))
                              (define %1 (+  %0.new0  15))
                              (define %2 (extract  %1 %0.new0 Vv))
                              (define %3 (bvusat %2 16 8 ))
                              %3
                              )
                    )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vavguwrnd_128B  %arg1 %arg0 Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvadd  %2  %5))
                            (define %7 (bvadd  %6  %arg1))
                            (define %8 (bvudiv  %7  %arg0))
                            %8
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyhv_acc_128B  Vxx Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg1))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %arg0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %8 (+  %7  %lastidx1))
                            (define %9 (extract  %8 %7 Vu))
                            (define %14 (extract  %8 %7 Vv))
                            (define %15 (sign-extend  %9 (bitvector %arg0)))
                            (define %16 (sign-extend  %14 (bitvector %arg0)))
                            (define %17 (bvmul  %15  %16))
                            (define %18 (bvadd  %4  %17))
                            %18
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vunpackob_128B  %arg1 Vxx Vu %elemsize0 %arg0 )
  (define %dst0
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %factor0 (/  %arg0  %elemsize0))
                            (define %0.new0 (*  i.new  %factor0))
                            (define %lastidx0 (-  %arg0  1))
                            (define %1 (+  %0.new0  %lastidx0))
                            (define %2 (extract  %1 %0.new0 Vxx))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %4 (+  i.new  %lastidx1))
                            (define %5 (extract  %4 i.new Vu))
                            (define %6 (bvshl  %5  %arg1))
                            (define %7 (zero-extend  %6 (bitvector %arg0)))
                            (define %8 (bvor  %2  %7))
                            %8
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vmpybusv_acc_128B  Vxx Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg1))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %arg0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %8 (+  %7  %lastidx1))
                            (define %9 (extract  %8 %7 Vu))
                            (define %14 (extract  %8 %7 Vv))
                            (define %15 (zero-extend  %9 (bitvector %arg0)))
                            (define %16 (sign-extend  %14 (bitvector %arg0)))
                            (define %17 (bvmul  %15  %16))
                            (define %18 (bvadd  %4  %17))
                            %18
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vdmpyhisat_acc_128B  Vx Vuu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define %1.clone.0 (+  i.new  31))
                (define %2.clone.0 (extract  %1.clone.0 i.new Vx))
                (define %dst0 %2.clone.0)
                (define %15.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 16))])
                              (define iterator.0.new.div (/  iterator.0.new  16))
                              (define %35.new0 (-  1  iterator.0.new.div))
                              (define %3 (*  16  %35.new0))
                              (define %5.new0 (*  iterator.0.new  64))
                              (define %6 (+  %3  i.new))
                              (define %7 (+  %6  %5.new0))
                              (define %8 (+  %7  15))
                              (define %9 (extract  %8 %7 Vuu))
                              (define %11 (+  iterator.0.new  15))
                              (define %12 (extract  %11 iterator.0.new Rt))
                              (define %13 (sign-extend  %9 (bitvector 32)))
                              (define %14 (sign-extend  %12 (bitvector 32)))
                              (define %15 (bvmul  %13  %14))
                              %15
                              )
                    ))
                (define %33.clone.2 (+  i.new  31))
                (define %15.acc0 (bvaddnsw %15.ext0.red %dst0 32))
                %15.acc0
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vassign_128B  Vu )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 32))])
                            (define %1 (+  i.new  31))
                            (define %2 (extract  %1 i.new Vu))
                            %2
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vaddwq_128B  Vu.norm Vx.norm QvV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define i.new.div.norm (/  i.new.norm  %arg2.norm))
                            (define %0.norm (extract  i.new.div.norm i.new.div.norm QvV.norm))
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %2.norm (+  i.new.norm  %lastidx0.norm))
                            (define %3.norm (extract  %2.norm i.new.norm Vx.norm))
                            (define %9.norm (extract  %2.norm i.new.norm Vu.norm))
                            (define %10.norm (bvadd  %3.norm  %9.norm))
                            (define %cond0.norm (bveq  %0.norm  (bv 1 1)))
                            (define %11.norm (if (equal? %cond0.norm #t) %3.norm %10.norm))
                            %11.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vaddhw_128B  Vu Vv )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 16))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (extract  %3 %2 Vv))
                            (define %10 (bvadd  %4  %9))
                            (define %15 (sign-extend  %10 (bitvector 32)))
                            %15
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vgtuh_xor_128B  Vu.norm %arg1.norm Vv.norm %arg0.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %5.new0.norm (+  i.norm  %lastidx0.norm))
                            (define %6.norm (extract  %5.new0.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bvugt  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvxor  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_lvsplath_128B  Rt %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %2.high.idx 31 )
                            (define %2.low.idx (+ (- %2.high.idx %elemsize0) 1) )
                            (define %2 (extract  %2.high.idx %2.low.idx Rt))
                            %2
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmux_128B  QtV.norm Vv.norm Vu.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 8))])
                            (define i.new.div.norm (/  i.new.norm  8))
                            (define %0.norm (extract  i.new.div.norm i.new.div.norm QtV.norm))
                            (define %2.norm (+  i.new.norm  7))
                            (define %3.norm (extract  %2.norm i.new.norm Vu.norm))
                            (define %6.norm (extract  %2.norm i.new.norm Vv.norm))
                            (define %cond0.norm (bveq  %0.norm  (bv 1 1)))
                            (define %7.norm (if (equal? %cond0.norm #t) %3.norm %6.norm))
                            %7.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vdealb4w_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (concat
                  (apply
                    concat
                    (for/list ([iterator.1.new (reverse (range 16 32 8))])
                              (define %42.new0 (/  iterator.1.new  4))
                              (define %43.new0 (-  %42.new0  4))
                              (define %20 (*  8  %43.new0))
                              (define %22 (+  %20  i.new))
                              (define %23 (+  %22  7))
                              (define %24 (extract  %23 %22 Vu))
                              %24
                              )
                    )
                  (apply
                    concat
                    (for/list ([iterator.0.new (reverse (range 0 16 8))])
                              (define %0.new0 (*  iterator.0.new  2))
                              (define %2 (+  %0.new0  i.new))
                              (define %3 (+  %2  7))
                              (define %4 (extract  %3 %2 Vv))
                              %4
                              )
                    )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vrmpyubv_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define Vd (bv #b0 32))
                (define %15.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 8))])
                              (define %2 (+  iterator.0.new  i.new))
                              (define %3 (+  %2  7))
                              (define %4 (extract  %3 %2 Vu))
                              (define %9 (extract  %3 %2 Vv))
                              (define %10 (zero-extend  %4 (bitvector 16)))
                              (define %11 (zero-extend  %9 (bitvector 16)))
                              (define %12 (bvmul  %10  %11))
                              (define %15 (zero-extend  %12 (bitvector 32)))
                              %15
                              )
                    ))
                (define %75.clone.2 (+  i.new  31))
                (define %15.acc0 (bvadd %15.ext0.red Vd))
                %15.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vshuffb_128B  Vu %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0.new0 (*  iterator.0.new  %arg1))
                            (define i.new.div.copy.0 (/  i.new  %arg0))
                            (define %2 (+  i.new.div.copy.0  %1.new0.new0))
                            (define %3 (*  %elemsize0  %2))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %4 (+  %3  %lastidx1))
                            (define %5 (extract  %4 %3 Vu))
                            %5
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyiewuh_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vu))
                            (define %6 (+  %0.new0  15))
                            (define %7 (extract  %6 %0.new0 Vv))
                            (define %9 (sign-extend  %2 (bitvector 64)))
                            (define %10 (sign-extend  %7 (bitvector 64)))
                            (define %11 (bvmul  %9  %10))
                            (define %14.high.idx 63 )
                            (define %14.low.idx (+ (- %14.high.idx 32) 1) )
                            (define %14 (extract  %14.high.idx %14.low.idx %11))
                            %14
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vshufeh_128B  Vu Vv %elemsize0 %arg0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 %elemsize0))])
                            (define %factor0 (/  %elemsize0  %elemsize0))
                            (define %factor1 (*  2  %factor0))
                            (define %1.new0 (*  i.new  %factor1))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %1.new0  %lastidx1))
                            (define %4 (extract  %3 %1.new0 Vv))
                            (define %13 (extract  %3 %1.new0 Vu))
                            (concat %13 %4)
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vsubw_128B  Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvsub  %2  %5))
                            %6
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_pred_not_128B  QsV %innerlanesize0 %elemsize0 )
  (define QdV
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %highidx1 (+  i  %lastidx1))
                            (define %0 (extract  %highidx1 i QsV))
                            (define %1 (bvnot  %0))
                            %1
                            )
                  )
                )
      )
    )
  QdV
  )

(define (hexagon_V6_vmpybv_128B  Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (extract  %3 %2 Vv))
                            (define %10 (sign-extend  %4 (bitvector %arg0)))
                            (define %11 (sign-extend  %9 (bitvector %arg0)))
                            (define %12 (bvmul  %10  %11))
                            %12
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmaxub_128B  Vu.norm Vv.norm %elemsize0.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %1.norm (+  i.new.norm  %lastidx0.norm))
                            (define %2.norm (extract  %1.norm i.new.norm Vu.norm))
                            (define %5.norm (extract  %1.norm i.new.norm Vv.norm))
                            (define %6.norm (bvugt  %2.norm  %5.norm))
                            (define %13.norm (if (equal? %6.norm #t) %2.norm %5.norm))
                            %13.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vmpabus_acc_128B  Vxx Vuu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  128))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %8 (+  iterator.0.new  %0.new0))
                            (define %10 (+  %8  7))
                            (define %11 (extract  %10 %8 Vuu))
                            (define %12.new0 (*  iterator.0.new  2))
                            (define %13 (+  %12.new0  7))
                            (define %14 (extract  %13 %12.new0 Rt))
                            (define %15 (zero-extend  %11 (bitvector 16)))
                            (define %16 (sign-extend  %14 (bitvector 16)))
                            (define %17 (bvmul  %15  %16))
                            (define %22 (+  %8  1024))
                            (define %23 (+  %22  7))
                            (define %24 (extract  %23 %22 Vuu))
                            (define %76.new0 (/  iterator.0.new  4))
                            (define %77 (+  %76.new0  1))
                            (define %25 (*  8  %77))
                            (define %26 (+  %25  7))
                            (define %27 (extract  %26 %25 Rt))
                            (define %28 (zero-extend  %24 (bitvector 16)))
                            (define %29 (sign-extend  %27 (bitvector 16)))
                            (define %30 (bvmul  %28  %29))
                            (define %31 (bvadd  %17  %30))
                            (define %32 (bvadd  %4  %31))
                            %32
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vswap_128B  QtV.norm Vu.norm Vv.norm )
  (define Vdd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.norm (reverse (range 0 128 1))])
                              (define %0.copy0.copy.0.norm (extract  %iterator1.norm %iterator1.norm QtV.norm))
                              (define %13.norm (bvnot  %0.copy0.copy.0.norm))
                              (define %14.norm (*  8  %iterator1.norm))
                              (define %15.norm (+  %14.norm  7))
                              (define %16.norm (extract  %15.norm %14.norm Vu.norm))
                              (define %19.norm (extract  %15.norm %14.norm Vv.norm))
                              (define %20.norm (if (equal? %13.norm (bv #b1 1))
                                                 %16.norm %19.norm))
                              %20.norm
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new.norm (reverse (range 0 1024 8))])
                              (define %iterator0.new.div.norm (/  %iterator0.new.norm  8))
                              (define %0.norm (extract  %iterator0.new.div.norm %iterator0.new.div.norm QtV.norm))
                              (define %2.norm (+  %iterator0.new.norm  7))
                              (define %3.norm (extract  %2.norm %iterator0.new.norm Vu.norm))
                              (define %6.norm (extract  %2.norm %iterator0.new.norm Vv.norm))
                              (define %cond0.norm (bveq  %0.norm  (bv 1 1)))
                              (define %7.norm (if (equal? %cond0.norm #t) %3.norm %6.norm))
                              %7.norm
                              )
                    )
                  )
                )
      )
    )
  Vdd.norm
  )

(define (hexagon_V6_vadduwsat_dv_128B  Vuu Vvv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg0))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vuu))
                            (define %9 (extract  %3 %2 Vvv))
                            (define %10 (bvaddnuw  %4  %9 %elemsize0))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vgtuw_or_128B  %arg1.norm Vv.norm Vu.norm %arg0.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %5.new0.norm (+  i.norm  %lastidx0.norm))
                            (define %6.norm (extract  %5.new0.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bvugt  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvor  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vmpauhb_128B  Vuu Rt )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define %0.new0 (*  iterator.0.new  2))
                            (define %1.new0 (/  i.new  2))
                            (define %3 (+  %0.new0  %1.new0))
                            (define %5 (+  %3  15))
                            (define %6 (extract  %5 %3 Vuu))
                            (define %8 (+  %0.new0  7))
                            (define %9 (extract  %8 %0.new0 Rt))
                            (define %11 (sign-extend  %6 (bitvector 32)))
                            (define %12 (sign-extend  %9 (bitvector 32)))
                            (define %13 (bvmul  %11  %12))
                            (define %18 (+  %3  1024))
                            (define %19 (+  %18  15))
                            (define %20 (extract  %19 %18 Vuu))
                            (define %68.new0 (/  iterator.0.new  4))
                            (define %69 (+  %68.new0  1))
                            (define %21 (*  8  %69))
                            (define %22 (+  %21  7))
                            (define %23 (extract  %22 %21 Rt))
                            (define %25 (sign-extend  %20 (bitvector 32)))
                            (define %26 (sign-extend  %23 (bitvector 32)))
                            (define %27 (bvmul  %25  %26))
                            (define %28 (bvadd  %13  %27))
                            %28
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vavgwrnd_128B  %arg1 %arg0 Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvadd  %2  %5))
                            (define %7 (bvadd  %6  %arg1))
                            (define %8 (bvsdiv  %7  %arg0))
                            %8
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vsububsat_dv_128B  Vuu Vvv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg0))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vuu))
                            (define %9 (extract  %3 %2 Vvv))
                            (define %10 (bvsubnuw  %4  %9 %elemsize0))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmpabuu_acc_128B  Vxx Vuu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  128))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %8 (+  iterator.0.new  %0.new0))
                            (define %10 (+  %8  7))
                            (define %11 (extract  %10 %8 Vuu))
                            (define %12.new0 (*  iterator.0.new  2))
                            (define %13 (+  %12.new0  7))
                            (define %14 (extract  %13 %12.new0 Rt))
                            (define %15 (zero-extend  %11 (bitvector 16)))
                            (define %16 (zero-extend  %14 (bitvector 16)))
                            (define %17 (bvmul  %15  %16))
                            (define %22 (+  %8  1024))
                            (define %23 (+  %22  7))
                            (define %24 (extract  %23 %22 Vuu))
                            (define %76.new0 (/  iterator.0.new  4))
                            (define %77 (+  %76.new0  1))
                            (define %25 (*  8  %77))
                            (define %26 (+  %25  7))
                            (define %27 (extract  %26 %25 Rt))
                            (define %28 (zero-extend  %24 (bitvector 16)))
                            (define %29 (zero-extend  %27 (bitvector 16)))
                            (define %30 (bvmul  %28  %29))
                            (define %31 (bvadd  %17  %30))
                            (define %32 (bvadd  %4  %31))
                            %32
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vminuh_128B  Vv.norm Vu.norm %elemsize0.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %1.norm (+  i.new.norm  %lastidx0.norm))
                            (define %2.norm (extract  %1.norm i.new.norm Vu.norm))
                            (define %5.norm (extract  %1.norm i.new.norm Vv.norm))
                            (define %6.norm (bvult  %2.norm  %5.norm))
                            (define %13.norm (if (equal? %6.norm #t) %2.norm %5.norm))
                            %13.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vrmpybv_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define Vd (bv #b0 32))
                (define %15.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 8))])
                              (define %2 (+  iterator.0.new  i.new))
                              (define %3 (+  %2  7))
                              (define %4 (extract  %3 %2 Vu))
                              (define %9 (extract  %3 %2 Vv))
                              (define %10 (sign-extend  %4 (bitvector 16)))
                              (define %11 (sign-extend  %9 (bitvector 16)))
                              (define %12 (bvmul  %10  %11))
                              (define %15 (sign-extend  %12 (bitvector 32)))
                              %15
                              )
                    ))
                (define %75.clone.2 (+  i.new  31))
                (define %15.acc0 (bvadd %15.ext0.red Vd))
                %15.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyub_128B  Vu Rt )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define iterator.0.new.div (/  iterator.0.new  8))
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %3 (+  %2  7))
                            (define %4 (extract  %3 %2 Vu))
                            (define %5.new0 (/  i.new  16))
                            (define %6 (+  %5.new0  iterator.0.new.div))
                            (define %7 (modulo  %6  4))
                            (define %8 (*  8  %7))
                            (define %9 (+  %8  7))
                            (define %10 (extract  %9 %8 Rt))
                            (define %11 (zero-extend  %4 (bitvector 16)))
                            (define %12 (zero-extend  %10 (bitvector 16)))
                            (define %13 (bvmul  %11  %12))
                            %13
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_veqb_or_128B  %arg1.norm Vv.norm Vu.norm %arg0.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx2.norm (-  %elemsize0.norm  1))
                            (define %highidx1.norm (+  i.norm  %lastidx2.norm))
                            (define %6.norm (extract  %highidx1.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bveq  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvor  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vmpyiwub_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 256 8))])
                            (define i.new.div (/  i.new  8))
                            (define %0.new0 (*  i.new  4))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vx))
                            (define %5 (extract  %1 %0.new0 Vu))
                            (define %6 (modulo  i.new.div  4))
                            (define %7 (*  8  %6))
                            (define %8 (+  %7  7))
                            (define %9 (extract  %8 %7 Rt))
                            (define %11 (sign-extend  %5 (bitvector 64)))
                            (define %12 (zero-extend  %9 (bitvector 64)))
                            (define %13 (bvmul  %11  %12))
                            (define %14 (sign-extend  %2 (bitvector 64)))
                            (define %15 (bvadd  %14  %13))
                            (define %18.high.idx 63 )
                            (define %18.low.idx (+ (- %18.high.idx 32) 1) )
                            (define %18 (extract  %18.high.idx %18.low.idx %15))
                            %18
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vmpyiwub_128B  Vu Rt )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 256 8))])
                            (define i.new.div (/  i.new  8))
                            (define %0.new0 (*  i.new  4))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vu))
                            (define %3 (modulo  i.new.div  4))
                            (define %4 (*  8  %3))
                            (define %5 (+  %4  7))
                            (define %6 (extract  %5 %4 Rt))
                            (define %8 (sign-extend  %2 (bitvector 64)))
                            (define %9 (zero-extend  %6 (bitvector 64)))
                            (define %10 (bvmul  %8  %9))
                            (define %13.high.idx 63 )
                            (define %13.low.idx (+ (- %13.high.idx 32) 1) )
                            (define %13 (extract  %13.high.idx %13.low.idx %10))
                            %13
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vsubuhw_128B  Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (extract  %3 %2 Vv))
                            (define %10 (bvsub  %4  %9))
                            (define %15 (zero-extend  %10 (bitvector %arg0)))
                            %15
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmpyuhe_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vx))
                            (define %6 (+  %0.new0  15))
                            (define %7 (extract  %6 %0.new0 Vu))
                            (define %10 (extract  15 0 Rt))
                            (define %11 (zero-extend  %7 (bitvector 32)))
                            (define %12 (zero-extend  %10 (bitvector 32)))
                            (define %13 (bvmul  %11  %12))
                            (define %14 (bvadd  %2  %13))
                            %14
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vdmpyhvsat_acc_128B  Vx Vu Vv )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define %28.clone.0 (+  i.new  31))
                (define %29.clone.0 (extract  %28.clone.0 i.new Vx))
                (define %dst0 %29.clone.0)
                (define %12.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 16))])
                              (define %2 (+  iterator.0.new  i.new))
                              (define %3 (+  %2  15))
                              (define %4 (extract  %3 %2 Vu))
                              (define %9 (extract  %3 %2 Vv))
                              (define %10 (sign-extend  %4 (bitvector 32)))
                              (define %11 (sign-extend  %9 (bitvector 32)))
                              (define %12 (bvmul  %10  %11))
                              %12
                              )
                    ))
                (define %33.clone.1 (+  i.new  31))
                (define %12.acc0 (bvaddnsw %12.ext0.red %dst0 32))
                %12.acc0
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vgtb_or_128B  %arg0.norm Vu.norm Vv.norm %arg1.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx2.norm (-  %elemsize0.norm  1))
                            (define %highidx1.norm (+  i.norm  %lastidx2.norm))
                            (define %6.norm (extract  %highidx1.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bvsgt  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvor  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vpackwh_sat_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.new (reverse (range 0 512 16))])
                              (define %6.new0 (*  %iterator1.new  2))
                              (define %7 (+  %6.new0  31))
                              (define %8 (extract  %7 %6.new0 Vu))
                              (define %9 (bvssat %8 32 16 ))
                              %9
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new (reverse (range 0 512 16))])
                              (define %0.new0 (*  %iterator0.new  2))
                              (define %1 (+  %0.new0  31))
                              (define %2 (extract  %1 %0.new0 Vv))
                              (define %3 (bvssat %2 32 16 ))
                              %3
                              )
                    )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vrmpybusv_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define Vd (bv #b0 32))
                (define %15.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 8))])
                              (define %2 (+  iterator.0.new  i.new))
                              (define %3 (+  %2  7))
                              (define %4 (extract  %3 %2 Vu))
                              (define %9 (extract  %3 %2 Vv))
                              (define %10 (zero-extend  %4 (bitvector 16)))
                              (define %11 (sign-extend  %9 (bitvector 16)))
                              (define %12 (bvmul  %10  %11))
                              (define %15 (sign-extend  %12 (bitvector 32)))
                              %15
                              )
                    ))
                (define %75.clone.2 (+  i.new  31))
                (define %15.acc0 (bvadd %15.ext0.red Vd))
                %15.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vabsdiffw_128B  Vu.norm Vv.norm %elemsize0.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %1.norm (+  i.new.norm  %lastidx0.norm))
                            (define %2.norm (extract  %1.norm i.new.norm Vu.norm))
                            (define %5.norm (extract  %1.norm i.new.norm Vv.norm))
                            (define %6.norm (bvsgt  %2.norm  %5.norm))
                            (define %13.norm (bvsub  %2.norm  %5.norm))
                            (define %20.norm (bvsub  %5.norm  %2.norm))
                            (define %21.norm (if (equal? %6.norm #t) %13.norm %20.norm))
                            %21.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vmpyihb_acc_128B  Vx Vu Rt %innerlanesize0 %elemsize0 %arg0 %arg1 %arg2 %arg3 )
  (define %dst0
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 32 32))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define i.new.div (/  i.new  %arg3))
                            (define %factor0 (/  %arg0  %elemsize0))
                            (define %0.new0 (*  i.new  %factor0))
                            (define %lastidx0 (-  %arg0  1))
                            (define %1 (+  %0.new0  %lastidx0))
                            (define %2 (extract  %1 %0.new0 Vx))
                            (define %5 (extract  %1 %0.new0 Vu))
                            (define %6 (modulo  i.new.div  %arg2))
                            (define %7 (*  %elemsize0  %6))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %8 (+  %7  %lastidx1))
                            (define %9 (extract  %8 %7 Rt))
                            (define %11 (sign-extend  %5 (bitvector %arg1)))
                            (define %12 (sign-extend  %9 (bitvector %arg1)))
                            (define %13 (bvmul  %11  %12))
                            (define %14 (sign-extend  %2 (bitvector %arg1)))
                            (define %15 (bvadd  %14  %13))
                            (define %18.high.idx (- %arg1 1))
                            (define %18.low.idx (+ (- %18.high.idx %arg0) 1) )
                            (define %18 (extract  %18.high.idx %18.low.idx %15))
                            %18
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vmpyiewuh_acc_128B  Vx Vu Vv )
  (define %dst0
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vx))
                            (define %5 (extract  %1 %0.new0 Vu))
                            (define %9 (+  %0.new0  15))
                            (define %10 (extract  %9 %0.new0 Vv))
                            (define %12 (sign-extend  %5 (bitvector 64)))
                            (define %13 (sign-extend  %10 (bitvector 64)))
                            (define %14 (bvmul  %12  %13))
                            (define %15 (sign-extend  %2 (bitvector 64)))
                            (define %16 (bvadd  %15  %14))
                            (define %19.high.idx 63 )
                            (define %19.low.idx (+ (- %19.high.idx 32) 1) )
                            (define %19 (extract  %19.high.idx %19.low.idx %16))
                            %19
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vshuffob_128B  Vu Vv %elemsize0 %arg0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 %elemsize0))])
                            (define %factor0 (/  %elemsize0  %elemsize0))
                            (define %factor1 (*  2  %factor0))
                            (define %1.new0 (*  i.new  %factor1))
                            (define %2 (+  %arg0  %1.new0))
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vv))
                            (define %13 (extract  %3 %2 Vu))
                            (concat %13 %4)
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vdealb_128B  Vu %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %2 (+  iterator.0.new  i.new))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vu))
                            %4
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpybus_acc_128B  Vxx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define iterator.0.new.div (/  iterator.0.new  8))
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  128))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %8 (+  %7  7))
                            (define %9 (extract  %8 %7 Vu))
                            (define %10.new0 (/  i.new  16))
                            (define %11 (+  %10.new0  iterator.0.new.div))
                            (define %12 (modulo  %11  4))
                            (define %13 (*  8  %12))
                            (define %14 (+  %13  7))
                            (define %15 (extract  %14 %13 Rt))
                            (define %16 (zero-extend  %9 (bitvector 16)))
                            (define %17 (sign-extend  %15 (bitvector 16)))
                            (define %18 (bvmul  %16  %17))
                            (define %19 (bvadd  %4  %18))
                            %19
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vdmpyhsat_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define %1.clone.0 (+  i.new  31))
                (define %2.clone.0 (extract  %1.clone.0 i.new Vx))
                (define %dst0 %2.clone.0)
                (define %13.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 16))])
                              (define %5 (+  iterator.0.new  i.new))
                              (define %6 (+  %5  15))
                              (define %7 (extract  %6 %5 Vu))
                              (define %9 (+  iterator.0.new  15))
                              (define %10 (extract  %9 iterator.0.new Rt))
                              (define %11 (sign-extend  %7 (bitvector 32)))
                              (define %12 (sign-extend  %10 (bitvector 32)))
                              (define %13 (bvmul  %11  %12))
                              %13
                              )
                    ))
                (define %29.clone.2 (+  i.new  31))
                (define %13.acc0 (bvaddnsw %13.ext0.red %dst0 32))
                %13.acc0
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vmpabuu_128B  Vuu Rt )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define %1.new0 (/  i.new  2))
                            (define %3 (+  iterator.0.new  %1.new0))
                            (define %5 (+  %3  7))
                            (define %6 (extract  %5 %3 Vuu))
                            (define %7.new0 (*  iterator.0.new  2))
                            (define %8 (+  %7.new0  7))
                            (define %9 (extract  %8 %7.new0 Rt))
                            (define %10 (zero-extend  %6 (bitvector 16)))
                            (define %11 (zero-extend  %9 (bitvector 16)))
                            (define %12 (bvmul  %10  %11))
                            (define %17 (+  %3  1024))
                            (define %18 (+  %17  7))
                            (define %19 (extract  %18 %17 Vuu))
                            (define %64.new0 (/  iterator.0.new  4))
                            (define %65 (+  %64.new0  1))
                            (define %20 (*  8  %65))
                            (define %21 (+  %20  7))
                            (define %22 (extract  %21 %20 Rt))
                            (define %23 (zero-extend  %19 (bitvector 16)))
                            (define %24 (zero-extend  %22 (bitvector 16)))
                            (define %25 (bvmul  %23  %24))
                            (define %26 (bvadd  %12  %25))
                            %26
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vrmpybus_128B  Vu Rt )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define Vd (bv #b0 32))
                (define %13.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 8))])
                              (define %2 (+  iterator.0.new  i.new))
                              (define %3 (+  %2  7))
                              (define %4 (extract  %3 %2 Vu))
                              (define %6 (+  iterator.0.new  7))
                              (define %7 (extract  %6 iterator.0.new Rt))
                              (define %8 (zero-extend  %4 (bitvector 16)))
                              (define %9 (sign-extend  %7 (bitvector 16)))
                              (define %10 (bvmul  %8  %9))
                              (define %13 (sign-extend  %10 (bitvector 32)))
                              %13
                              )
                    ))
                (define %67.clone.2 (+  i.new  31))
                (define %13.acc0 (bvadd %13.ext0.red Vd))
                %13.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyubv_128B  Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (extract  %3 %2 Vv))
                            (define %10 (zero-extend  %4 (bitvector %arg0)))
                            (define %11 (zero-extend  %9 (bitvector %arg0)))
                            (define %12 (bvmul  %10  %11))
                            %12
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmpauhb_acc_128B  Vxx Vuu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  128))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  31))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %5.new0 (*  iterator.0.new  2))
                            (define %8 (+  %5.new0  %0.new0))
                            (define %10 (+  %8  15))
                            (define %11 (extract  %10 %8 Vuu))
                            (define %13 (+  %5.new0  7))
                            (define %14 (extract  %13 %5.new0 Rt))
                            (define %16 (sign-extend  %11 (bitvector 32)))
                            (define %17 (sign-extend  %14 (bitvector 32)))
                            (define %18 (bvmul  %16  %17))
                            (define %23 (+  %8  1024))
                            (define %24 (+  %23  15))
                            (define %25 (extract  %24 %23 Vuu))
                            (define %80.new0 (/  iterator.0.new  4))
                            (define %81 (+  %80.new0  1))
                            (define %26 (*  8  %81))
                            (define %27 (+  %26  7))
                            (define %28 (extract  %27 %26 Rt))
                            (define %30 (sign-extend  %25 (bitvector 32)))
                            (define %31 (sign-extend  %28 (bitvector 32)))
                            (define %32 (bvmul  %30  %31))
                            (define %33 (bvadd  %18  %32))
                            (define %34 (bvadd  %4  %33))
                            %34
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vgtuh_128B  %arg1.norm %arg0.norm Vv.norm Vu.norm %elemsize0.norm %arg2.norm )
  (define QdV.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %3.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %4.norm (+  %3.new0.norm  %lastidx1.norm))
                            (define %5.norm (extract  %4.norm %3.new0.norm Vu.norm))
                            (define %9.norm (extract  %4.norm %3.new0.norm Vv.norm))
                            (define %10.norm (bvugt  %5.norm  %9.norm))
                            (define %11.norm (if (equal? %10.norm #t) %arg0.norm %arg1.norm))
                            %11.norm
                            )
                  )
                )
      )
    )
  QdV.norm
  )

(define (hexagon_V6_vmpyih_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 16))])
                            (define %1 (+  i.new  15))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (sign-extend  %2 (bitvector 32)))
                            (define %7 (sign-extend  %5 (bitvector 32)))
                            (define %8 (bvmul  %6  %7))
                            (define %11.high.idx 31 )
                            (define %11.low.idx (+ (- %11.high.idx 16) 1) )
                            (define %11 (extract  %11.high.idx %11.low.idx %8))
                            %11
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vnavgub_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 8))])
                            (define %1 (+  i.new  7))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvsub  %2  %5))
                            (define %7 (bvudiv  %6  (bv 2 8)))
                            %7
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vpackeh_128B  Vu Vv %elemsize0 %elemsize1 %arg1 %arg2 %arg3 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.new (reverse (range 0 512 %elemsize1))])
                              (define %factor2 (/  %elemsize1  %elemsize1))
                              (define %factor3 (*  2  %factor2))
                              (define %11.new0 (*  %iterator1.new  %factor3))
                              (define %lastidx3 (-  %elemsize1  1))
                              (define %13 (+  %11.new0  %lastidx3))
                              (define %14 (extract  %13 %11.new0 Vu))
                              %14
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new (reverse (range 0 512 %elemsize0))])
                              (define %factor0 (/  %elemsize0  %elemsize0))
                              (define %factor1 (*  2  %factor0))
                              (define %1.new0 (*  %iterator0.new  %factor1))
                              (define %lastidx1 (-  %elemsize0  1))
                              (define %3 (+  %1.new0  %lastidx1))
                              (define %4 (extract  %3 %1.new0 Vv))
                              %4
                              )
                    )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vgth_xor_128B  Vu.norm Vv.norm %arg1.norm %arg0.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %5.new0.norm (+  i.norm  %lastidx0.norm))
                            (define %6.norm (extract  %5.new0.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bvsgt  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvxor  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vsubb_dv_128B  Vuu Vvv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg0))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vuu))
                            (define %9 (extract  %3 %2 Vvv))
                            (define %10 (bvsub  %4  %9))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmpyuhe_128B  Vu Rt )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %1.new0 (*  i.new  2))
                            (define %3 (+  %1.new0  15))
                            (define %4 (extract  %3 %1.new0 Vu))
                            (define %7 (extract  15 0 Rt))
                            (define %8 (zero-extend  %4 (bitvector 32)))
                            (define %9 (zero-extend  %7 (bitvector 32)))
                            (define %10 (bvmul  %8  %9))
                            %10
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vinsertwr_128B  Rt )
  (define Vx
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([%inner.it (reverse (range 0 32 32))])
                            Rt
                            )
                  )
                )
      )
    )
  Vx
  )

(define (hexagon_V6_vgtuw_and_128B  Vv.norm Vu.norm %arg0.norm %arg1.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %5.new0.norm (+  i.norm  %lastidx0.norm))
                            (define %6.norm (extract  %5.new0.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bvugt  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvand  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vrmpybus_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 8))])
                            (define %1 (+  i.new  31))
                            (define %2 (extract  %1 i.new Vx))
                            (define %5 (+  iterator.0.new  i.new))
                            (define %6 (+  %5  7))
                            (define %7 (extract  %6 %5 Vu))
                            (define %9 (+  iterator.0.new  7))
                            (define %10 (extract  %9 iterator.0.new Rt))
                            (define %11 (zero-extend  %7 (bitvector 16)))
                            (define %12 (sign-extend  %10 (bitvector 16)))
                            (define %13 (bvmul  %11  %12))
                            (define %14 (sign-extend  %13 (bitvector 32)))
                            (define %15 (bvadd  %2  %14))
                            %15
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vaddwsat_dv_128B  Vuu Vvv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg0))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vuu))
                            (define %9 (extract  %3 %2 Vvv))
                            (define %10 (bvaddnsw  %4  %9 %elemsize0))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vgth_and_128B  %arg0.norm Vv.norm Vu.norm %arg1.norm QxV.norm %elemsize0.norm %arg2.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %5.new0.norm (+  i.norm  %lastidx0.norm))
                            (define %6.norm (extract  %5.new0.norm i.norm QxV.norm))
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %8.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %9.norm (+  %8.new0.norm  %lastidx1.norm))
                            (define %10.norm (extract  %9.norm %8.new0.norm Vu.norm))
                            (define %14.norm (extract  %9.norm %8.new0.norm Vv.norm))
                            (define %15.norm (bvsgt  %10.norm  %14.norm))
                            (define %16.norm (if (equal? %15.norm #t) %arg0.norm %arg1.norm))
                            (define %17.norm (bvand  %6.norm  %16.norm))
                            %17.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vandqrt_128B  QuV.norm Rt.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 8))])
                            (define i.new.div.norm (/  i.new.norm  8))
                            (define %0.norm (extract  i.new.div.norm i.new.div.norm QuV.norm))
                            (define %1.norm (modulo  i.new.div.norm  4))
                            (define %2.norm (*  8  %1.norm))
                            (define %3.norm (+  %2.norm  7))
                            (define %4.norm (extract  %3.norm %2.norm Rt.norm))
                            (define %cond0.norm (bveq  %0.norm  (bv 1 1)))
                            (define %5.norm (if (equal? %cond0.norm #t) %4.norm (bv 0 8)))
                            %5.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vand_128B  Vu Vv %innerlanesize0 %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvand  %2  %5))
                            %6
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_veqw_128B  Vu.norm %arg0.norm Vv.norm %arg1.norm %elemsize0.norm %arg2.norm )
  (define QdV.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %3.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %4.norm (+  %3.new0.norm  %lastidx1.norm))
                            (define %5.norm (extract  %4.norm %3.new0.norm Vu.norm))
                            (define %9.norm (extract  %4.norm %3.new0.norm Vv.norm))
                            (define %10.norm (bveq  %5.norm  %9.norm))
                            (define %11.norm (if (equal? %10.norm #t) %arg0.norm %arg1.norm))
                            %11.norm
                            )
                  )
                )
      )
    )
  QdV.norm
  )

(define (hexagon_V6_vmpyh_128B  Vu Rt )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 16))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vu))
                            (define %6 (+  iterator.0.new  15))
                            (define %7 (extract  %6 iterator.0.new Rt))
                            (define %8 (sign-extend  %4 (bitvector 32)))
                            (define %9 (sign-extend  %7 (bitvector 32)))
                            (define %10 (bvmul  %8  %9))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmpyub_acc_128B  Vxx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define iterator.0.new.div (/  iterator.0.new  8))
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  128))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %8 (+  %7  7))
                            (define %9 (extract  %8 %7 Vu))
                            (define %10.new0 (/  i.new  16))
                            (define %11 (+  %10.new0  iterator.0.new.div))
                            (define %12 (modulo  %11  4))
                            (define %13 (*  8  %12))
                            (define %14 (+  %13  7))
                            (define %15 (extract  %14 %13 Rt))
                            (define %16 (zero-extend  %9 (bitvector 16)))
                            (define %17 (zero-extend  %15 (bitvector 16)))
                            (define %18 (bvmul  %16  %17))
                            (define %19 (bvadd  %4  %18))
                            %19
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vaddb_dv_128B  Vuu Vvv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg0))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vuu))
                            (define %9 (extract  %3 %2 Vvv))
                            (define %10 (bvadd  %4  %9))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vsubbsat_dv_128B  Vuu Vvv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg0))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vuu))
                            (define %9 (extract  %3 %2 Vvv))
                            (define %10 (bvsubnsw  %4  %9 %elemsize0))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vdmpybus_128B  Vu Rt )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 16))])
                (define Vd (bv #b0 16))
                (define %12.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 16 8))])
                              (define iterator.0.new.div (/  iterator.0.new  8))
                              (define %2 (+  iterator.0.new  i.new))
                              (define %3 (+  %2  7))
                              (define %4 (extract  %3 %2 Vu))
                              (define %5.new1 (/  i.new  8))
                              (define %5.new0 (+  %5.new1  iterator.0.new.div))
                              (define %6 (modulo  %5.new0  4))
                              (define %7 (*  8  %6))
                              (define %8 (+  %7  7))
                              (define %9 (extract  %8 %7 Rt))
                              (define %10 (zero-extend  %4 (bitvector 16)))
                              (define %11 (sign-extend  %9 (bitvector 16)))
                              (define %12 (bvmul  %10  %11))
                              %12
                              )
                    ))
                (define %34.clone.0 (+  i.new  15))
                (define %12.acc0 (bvadd %12.ext0.red Vd))
                %12.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyewuh_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vu))
                            (define %6 (+  %0.new0  15))
                            (define %7 (extract  %6 %0.new0 Vv))
                            (define %9 (sign-extend  %2 (bitvector 64)))
                            (define %10 (sign-extend  %7 (bitvector 64)))
                            (define %11 (bvmul  %9  %10))
                            (define %12 (bvlshr  %11  (bv 16 64)))
                            (define %15.high.idx 63 )
                            (define %15.low.idx (+ (- %15.high.idx 32) 1) )
                            (define %15 (extract  %15.high.idx %15.low.idx %12))
                            %15
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vandvrt_128B  Rt.norm Vu.norm )
  (define QdV.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 1))])
                            (define %0.norm (*  8  i.norm))
                            (define %1.norm (+  %0.norm  7))
                            (define %2.norm (extract  %1.norm %0.norm Vu.norm))
                            (define %3.norm (modulo  i.norm  4))
                            (define %4.norm (*  8  %3.norm))
                            (define %5.norm (+  %4.norm  7))
                            (define %6.norm (extract  %5.norm %4.norm Rt.norm))
                            (define %7.norm (bvand  %2.norm  %6.norm))
                            (define %8.norm (not (bveq  %7.norm  (bv 0 8) )))
                            (define %9.norm (if (equal? %8.norm #t) (bv 1 1) (bv 0 1)))
                            %9.norm
                            )
                  )
                )
      )
    )
  QdV.norm
  )

(define (hexagon_V6_vabsw_sat_128B  Vu %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %3 (integer->bitvector (abs (bitvector->integer %2)) (bitvector %elemsize0)))
                            (define %4 (bvssat %3 %elemsize0 %elemsize0 ))
                            %4
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vminh_128B  Vv.norm Vu.norm %elemsize0.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %1.norm (+  i.new.norm  %lastidx0.norm))
                            (define %2.norm (extract  %1.norm i.new.norm Vu.norm))
                            (define %5.norm (extract  %1.norm i.new.norm Vv.norm))
                            (define %6.norm (bvslt  %2.norm  %5.norm))
                            (define %13.norm (if (equal? %6.norm #t) %2.norm %5.norm))
                            %13.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vaddh_128B  Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvadd  %2  %5))
                            %6
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vrmpybusv_acc_128B  Vx Vu Vv )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 8))])
                            (define %1 (+  i.new  31))
                            (define %2 (extract  %1 i.new Vx))
                            (define %5 (+  iterator.0.new  i.new))
                            (define %6 (+  %5  7))
                            (define %7 (extract  %6 %5 Vu))
                            (define %12 (extract  %6 %5 Vv))
                            (define %13 (zero-extend  %7 (bitvector 16)))
                            (define %14 (sign-extend  %12 (bitvector 16)))
                            (define %15 (bvmul  %13  %14))
                            (define %16 (sign-extend  %15 (bitvector 32)))
                            (define %17 (bvadd  %2  %16))
                            %17
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vmpyieoh_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %1.new0 (*  i.new  2))
                            (define %3 (+  %1.new0  15))
                            (define %4 (extract  %3 %1.new0 Vu))
                            (define %7 (+  16  %1.new0))
                            (define %8 (+  %7  15))
                            (define %9 (extract  %8 %7 Vv))
                            (define %10 (sign-extend  %4 (bitvector 32)))
                            (define %11 (sign-extend  %9 (bitvector 32)))
                            (define %12 (bvmul  %10  %11))
                            (define %13 (bvshl  %12  (bv 16 32)))
                            %13
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vsh_128B  Vu %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (sign-extend  %4 (bitvector %arg0)))
                            %9
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vsatuwuh_128B  Vu Vv %elemsize0 %arg0 %arg1 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 %elemsize0))])
                            (define %factor0 (/  %arg1  %elemsize0))
                            (define %0.new0 (*  i.new  %factor0))
                            (define %lastidx1 (-  %arg1  1))
                            (define %1 (+  %0.new0  %lastidx1))
                            (define %2 (extract  %1 %0.new0 Vv))
                            (define %3 (bvusat %2 %arg1 %elemsize0 ))
                            (define %10 (extract  %1 %0.new0 Vu))
                            (define %11 (bvusat %10 %arg1 %elemsize0 ))
                            (concat %11 %3)
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyuhv_acc_128B  Vxx Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  %arg1))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %lastidx0 (-  %arg0  1))
                            (define %3 (+  %2  %lastidx0))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %8 (+  %7  %lastidx1))
                            (define %9 (extract  %8 %7 Vu))
                            (define %14 (extract  %8 %7 Vv))
                            (define %15 (zero-extend  %9 (bitvector %arg0)))
                            (define %16 (zero-extend  %14 (bitvector %arg0)))
                            (define %17 (bvmul  %15  %16))
                            (define %18 (bvadd  %4  %17))
                            %18
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vavguw_128B  %arg0 Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvadd  %2  %5))
                            (define %7 (bvudiv  %6  %arg0))
                            %7
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyhsat_acc_128B  Vxx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 16))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  64))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  31))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %8 (+  %7  15))
                            (define %9 (extract  %8 %7 Vu))
                            (define %11 (+  iterator.0.new  15))
                            (define %12 (extract  %11 iterator.0.new Rt))
                            (define %13 (sign-extend  %9 (bitvector 32)))
                            (define %14 (sign-extend  %12 (bitvector 32)))
                            (define %15 (bvmul  %13  %14))
                            (define %16 (bvaddnsw  %4  %15 32))
                            %16
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vnavgb_128B  %arg0 Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvsub  %2  %5))
                            (define %7 (bvsdiv  %6  %arg0))
                            %7
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyh_acc_128B  Vxx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 16))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  64))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  31))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %8 (+  %7  15))
                            (define %9 (extract  %8 %7 Vu))
                            (define %11 (+  iterator.0.new  15))
                            (define %12 (extract  %11 iterator.0.new Rt))
                            (define %13 (sign-extend  %9 (bitvector 32)))
                            (define %14 (sign-extend  %12 (bitvector 32)))
                            (define %15 (bvmul  %13  %14))
                            (define %16 (bvadd  %4  %15))
                            %16
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vsubhsat_128B  Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvsubnsw  %2  %5 %elemsize0))
                            %6
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyuh_acc_128B  Vxx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 16))])
                            (define %0.new0 (/  i.new  2))
                            (define %1.new0 (*  iterator.0.new  64))
                            (define %2 (+  %0.new0  %1.new0))
                            (define %3 (+  %2  31))
                            (define %4 (extract  %3 %2 Vxx))
                            (define %7 (+  iterator.0.new  %0.new0))
                            (define %8 (+  %7  15))
                            (define %9 (extract  %8 %7 Vu))
                            (define %11 (+  iterator.0.new  15))
                            (define %12 (extract  %11 iterator.0.new Rt))
                            (define %13 (zero-extend  %9 (bitvector 32)))
                            (define %14 (zero-extend  %12 (bitvector 32)))
                            (define %15 (bvmul  %13  %14))
                            (define %16 (bvadd  %4  %15))
                            %16
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vandvrt_acc_128B  Rt.norm QxV.norm Vu.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 1))])
                            (define %0.norm (extract  i.norm i.norm QxV.norm))
                            (define %1.norm (*  8  i.norm))
                            (define %2.norm (+  %1.norm  7))
                            (define %3.norm (extract  %2.norm %1.norm Vu.norm))
                            (define %4.norm (modulo  i.norm  4))
                            (define %5.norm (*  8  %4.norm))
                            (define %6.norm (+  %5.norm  7))
                            (define %7.norm (extract  %6.norm %5.norm Rt.norm))
                            (define %8.norm (bvand  %3.norm  %7.norm))
                            (define %9.norm (not (bveq  %8.norm  (bv 0 8) )))
                            (define %10.norm (if (equal? %9.norm #t) (bv 1 1) (bv 0 1)))
                            (define %11.norm (bvor  %0.norm  %10.norm))
                            %11.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vmpyowh_sacc_128B  Vx Vu Vv )
  (define %dst0
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vx))
                            (define %5 (extract  %1 %0.new0 Vu))
                            (define %8 (+  16  %0.new0))
                            (define %9 (+  %8  15))
                            (define %10 (extract  %9 %8 Vv))
                            (define %12 (sign-extend  %5 (bitvector 64)))
                            (define %13 (sign-extend  %10 (bitvector 64)))
                            (define %14 (bvmul  %12  %13))
                            (define %15 (sign-extend  %2 (bitvector 64)))
                            (define %16 (bvadd  %15  %14))
                            (define %17 (bvlshr  %16  (bv 14 64)))
                            (define %18 (bvadd  %17  (bv 1 64)))
                            (define %19 (bvlshr  %18  (bv 1 64)))
                            (define %20 (bvssat %19 64 32 ))
                            %20
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vpackwuh_sat_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.new (reverse (range 0 512 16))])
                              (define %6.new0 (*  %iterator1.new  2))
                              (define %7 (+  %6.new0  31))
                              (define %8 (extract  %7 %6.new0 Vu))
                              (define %9 (bvusat %8 32 16 ))
                              %9
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new (reverse (range 0 512 16))])
                              (define %0.new0 (*  %iterator0.new  2))
                              (define %1 (+  %0.new0  31))
                              (define %2 (extract  %1 %0.new0 Vv))
                              (define %3 (bvusat %2 32 16 ))
                              %3
                              )
                    )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vandvnqv_128B  Vu.norm QvV.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 1))])
                            (define %0.norm (extract  i.norm i.norm QvV.norm))
                            (define %1.norm (bvnot  %0.norm))
                            (define %2.norm (*  8  i.norm))
                            (define %3.norm (+  %2.norm  7))
                            (define %4.norm (extract  %3.norm %2.norm Vu.norm))
                            (define %5.norm (if (equal? %1.norm (bv #b1 1))
                                              %4.norm (bv 0 8)))
                            %5.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vdmpyhsusat_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define %1.clone.0 (+  i.new  31))
                (define %2.clone.0 (extract  %1.clone.0 i.new Vx))
                (define %dst0 %2.clone.0)
                (define %13.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 16))])
                              (define %5 (+  iterator.0.new  i.new))
                              (define %6 (+  %5  15))
                              (define %7 (extract  %6 %5 Vu))
                              (define %9 (+  iterator.0.new  15))
                              (define %10 (extract  %9 iterator.0.new Rt))
                              (define %11 (sign-extend  %7 (bitvector 32)))
                              (define %12 (zero-extend  %10 (bitvector 32)))
                              (define %13 (bvmul  %11  %12))
                              %13
                              )
                    ))
                (define %29.clone.2 (+  i.new  31))
                (define %13.acc0 (bvaddnsw %13.ext0.red %dst0 32))
                %13.acc0
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vdmpyhsuisat_128B  %arg0 Vuu Rt )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define %29.0 (+  i.new  31))
                (define %32 (extract  %29.0 i.new %arg0))
                (define Vd %32)
                (define %12.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 16))])
                              (define iterator.0.new.div (/  iterator.0.new  16))
                              (define %31.new0 (-  1  iterator.0.new.div))
                              (define %0 (*  16  %31.new0))
                              (define %2.new0 (*  iterator.0.new  64))
                              (define %3 (+  %0  i.new))
                              (define %4 (+  %3  %2.new0))
                              (define %5 (+  %4  15))
                              (define %6 (extract  %5 %4 Vuu))
                              (define %8 (+  iterator.0.new  15))
                              (define %9 (extract  %8 iterator.0.new Rt))
                              (define %10 (sign-extend  %6 (bitvector 32)))
                              (define %11 (zero-extend  %9 (bitvector 32)))
                              (define %12 (bvmul  %10  %11))
                              %12
                              )
                    ))
                (define %29.clone.0 (+  i.new  31))
                (define %12.acc0 (bvaddnsw %12.ext0.red Vd 32))
                %12.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vdmpyhb_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define iterator.0.new.div (/  iterator.0.new  8))
                            (define %1 (+  i.new  31))
                            (define %2 (extract  %1 i.new Vx))
                            (define %3.new0 (*  iterator.0.new  2))
                            (define %5 (+  %3.new0  i.new))
                            (define %6 (+  %5  15))
                            (define %7 (extract  %6 %5 Vu))
                            (define %8.new0 (/  i.new  16))
                            (define %9 (+  %8.new0  iterator.0.new.div))
                            (define %10 (modulo  %9  4))
                            (define %11 (*  8  %10))
                            (define %12 (+  %11  7))
                            (define %13 (extract  %12 %11 Rt))
                            (define %15 (sign-extend  %7 (bitvector 32)))
                            (define %16 (sign-extend  %13 (bitvector 32)))
                            (define %17 (bvmul  %15  %16))
                            (define %18 (bvadd  %2  %17))
                            %18
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vavgh_128B  %arg0 Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvadd  %2  %5))
                            (define %7 (bvsdiv  %6  %arg0))
                            %7
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vshufoeh_128B  Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vv))
                            (define %7.new0 (*  iterator.0.new  %arg1))
                            (define %15 (extract  %3 %2 Vu))
                            (concat %15 %4)
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vdmpyhb_128B  Vu Rt )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define Vd (bv #b0 32))
                (define %14.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 16 8))])
                              (define iterator.0.new.div (/  iterator.0.new  8))
                              (define %0.new0 (*  iterator.0.new  2))
                              (define %2 (+  %0.new0  i.new))
                              (define %3 (+  %2  15))
                              (define %4 (extract  %3 %2 Vu))
                              (define %5.new0 (/  i.new  16))
                              (define %6 (+  %5.new0  iterator.0.new.div))
                              (define %7 (modulo  %6  4))
                              (define %8 (*  8  %7))
                              (define %9 (+  %8  7))
                              (define %10 (extract  %9 %8 Rt))
                              (define %12 (sign-extend  %4 (bitvector 32)))
                              (define %13 (sign-extend  %10 (bitvector 32)))
                              (define %14 (bvmul  %12  %13))
                              %14
                              )
                    ))
                (define %37.clone.0 (+  i.new  31))
                (define %14.acc0 (bvadd %14.ext0.red Vd))
                %14.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpabuuv_128B  Vuu Vvv )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define %1.new0 (/  i.new  2))
                            (define %3 (+  iterator.0.new  %1.new0))
                            (define %5 (+  %3  7))
                            (define %6 (extract  %5 %3 Vuu))
                            (define %13 (extract  %5 %3 Vvv))
                            (define %14 (zero-extend  %6 (bitvector 16)))
                            (define %15 (zero-extend  %13 (bitvector 16)))
                            (define %16 (bvmul  %14  %15))
                            (define %21 (+  %3  1024))
                            (define %22 (+  %21  7))
                            (define %23 (extract  %22 %21 Vuu))
                            (define %30 (extract  %22 %21 Vvv))
                            (define %31 (zero-extend  %23 (bitvector 16)))
                            (define %32 (zero-extend  %30 (bitvector 16)))
                            (define %33 (bvmul  %31  %32))
                            (define %34 (bvadd  %16  %33))
                            %34
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vandnqrt_128B  Rt.norm QuV.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 1))])
                            (define %0.norm (extract  i.norm i.norm QuV.norm))
                            (define %1.norm (bvnot  %0.norm))
                            (define %2.norm (modulo  i.norm  4))
                            (define %3.norm (*  8  %2.norm))
                            (define %4.norm (+  %3.norm  7))
                            (define %5.norm (extract  %4.norm %3.norm Rt.norm))
                            (define %6.norm (if (equal? %1.norm (bv #b1 1))
                                              %5.norm (bv 0 8)))
                            %6.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vpackhb_sat_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.new (reverse (range 0 512 8))])
                              (define %9.new0 (*  %iterator1.new  2))
                              (define %10 (+  %9.new0  15))
                              (define %11 (extract  %10 %9.new0 Vu))
                              (define %12 (bvssat %11 16 8 ))
                              %12
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new (reverse (range 0 512 8))])
                              (define %0.new0 (*  %iterator0.new  2))
                              (define %1 (+  %0.new0  15))
                              (define %2 (extract  %1 %0.new0 Vv))
                              (define %3 (bvssat %2 16 8 ))
                              %3
                              )
                    )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_pred_and_n_128B  QsV QtV )
  (define QdV
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i (reverse (range 0 128 1))])
                            (define %0 (extract  i i QsV))
                            (define %1 (extract  i i QtV))
                            (define %2 (bvnot  %1))
                            (define %3 (bvand  %0  %2))
                            %3
                            )
                  )
                )
      )
    )
  QdV
  )

(define (hexagon_V6_vsububsat_128B  Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvsubnuw  %2  %5 %elemsize0))
                            %6
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyiowh_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vu))
                            (define %5 (+  16  %0.new0))
                            (define %6 (+  %5  15))
                            (define %7 (extract  %6 %5 Vv))
                            (define %9 (sign-extend  %2 (bitvector 64)))
                            (define %10 (sign-extend  %7 (bitvector 64)))
                            (define %11 (bvmul  %9  %10))
                            (define %14.high.idx 63 )
                            (define %14.low.idx (+ (- %14.high.idx 32) 1) )
                            (define %14 (extract  %14.high.idx %14.low.idx %11))
                            %14
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vandvqv_128B  QvV.norm Vu.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 8))])
                            (define i.new.div.norm (/  i.new.norm  8))
                            (define %0.norm (extract  i.new.div.norm i.new.div.norm QvV.norm))
                            (define %2.norm (+  i.new.norm  7))
                            (define %3.norm (extract  %2.norm i.new.norm Vu.norm))
                            (define %cond0.norm (bveq  %0.norm  (bv 1 1)))
                            (define %4.norm (if (equal? %cond0.norm #t) %3.norm (bv 0 8)))
                            %4.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vandnqrt_acc_128B  Rt.norm Vx.norm QuV.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 1))])
                            (define %0.norm (*  8  i.norm))
                            (define %1.norm (+  %0.norm  7))
                            (define %2.norm (extract  %1.norm %0.norm Vx.norm))
                            (define %3.norm (extract  i.norm i.norm QuV.norm))
                            (define %4.norm (bvnot  %3.norm))
                            (define %5.norm (modulo  i.norm  4))
                            (define %6.norm (*  8  %5.norm))
                            (define %7.norm (+  %6.norm  7))
                            (define %8.norm (extract  %7.norm %6.norm Rt.norm))
                            (define %9.norm (if (equal? %4.norm (bv #b1 1))
                                              %8.norm (bv 0 8)))
                            (define %10.norm (bvor  %2.norm  %9.norm))
                            %10.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vsubhw_128B  Vu Vv )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 16))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (extract  %3 %2 Vv))
                            (define %10 (bvsub  %4  %9))
                            (define %15 (sign-extend  %10 (bitvector 32)))
                            %15
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vadduwsat_128B  Vu Vv %elemsize0 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx0 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx0))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (extract  %1 i.new Vv))
                            (define %6 (bvaddnuw  %2  %5 %elemsize0))
                            %6
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_pred_or_128B  QsV QtV %innerlanesize0 %elemsize0 )
  (define QdV
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %lastidx2 (-  %elemsize0  1))
                            (define %highidx2 (+  i  %lastidx2))
                            (define %0 (extract  %highidx2 i QsV))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %highidx1 (+  i  %lastidx1))
                            (define %1 (extract  %highidx1 i QtV))
                            (define %2 (bvor  %0  %1))
                            %2
                            )
                  )
                )
      )
    )
  QdV
  )

(define (hexagon_V6_pred_xor_128B  QsV QtV %innerlanesize0 %elemsize0 )
  (define QdV
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %lastidx2 (-  %elemsize0  1))
                            (define %highidx2 (+  i  %lastidx2))
                            (define %0 (extract  %highidx2 i QsV))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %highidx1 (+  i  %lastidx1))
                            (define %1 (extract  %highidx1 i QtV))
                            (define %2 (bvxor  %0  %1))
                            %2
                            )
                  )
                )
      )
    )
  QdV
  )

(define (hexagon_V6_vaddubh_128B  Vu Vv %outerlanesize0 %innerlanesize0 %elemsize0 %arg0 %arg1 )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 %outerlanesize0))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %3 (+  %2  %lastidx1))
                            (define %4 (extract  %3 %2 Vu))
                            (define %9 (extract  %3 %2 Vv))
                            (define %10 (bvadd  %4  %9))
                            (define %15 (zero-extend  %10 (bitvector %arg0)))
                            %15
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmpyih_acc_128B  Vx Vu Vv )
  (define %dst0
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 16))])
                            (define %1 (+  i.new  15))
                            (define %2 (extract  %1 i.new Vx))
                            (define %5 (extract  %1 i.new Vu))
                            (define %8 (extract  %1 i.new Vv))
                            (define %9 (sign-extend  %5 (bitvector 32)))
                            (define %10 (sign-extend  %8 (bitvector 32)))
                            (define %11 (bvmul  %9  %10))
                            (define %12 (sign-extend  %2 (bitvector 32)))
                            (define %13 (bvadd  %12  %11))
                            (define %16.high.idx 31 )
                            (define %16.low.idx (+ (- %16.high.idx 16) 1) )
                            (define %16 (extract  %16.high.idx %16.low.idx %13))
                            %16
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vabsdiffub_128B  Vv.norm Vu.norm %elemsize0.norm )
  (define Vd.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 %elemsize0.norm))])
                            (define %lastidx0.norm (-  %elemsize0.norm  1))
                            (define %1.norm (+  i.new.norm  %lastidx0.norm))
                            (define %2.norm (extract  %1.norm i.new.norm Vu.norm))
                            (define %5.norm (extract  %1.norm i.new.norm Vv.norm))
                            (define %6.norm (bvugt  %2.norm  %5.norm))
                            (define %13.norm (bvsub  %2.norm  %5.norm))
                            (define %20.norm (bvsub  %5.norm  %2.norm))
                            (define %21.norm (if (equal? %6.norm #t) %13.norm %20.norm))
                            %21.norm
                            )
                  )
                )
      )
    )
  Vd.norm
  )

(define (hexagon_V6_vandqrt_acc_128B  Rt.norm QuV.norm Vx.norm )
  (define %dst0.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new.norm (reverse (range 0 1024 8))])
                            (define i.new.div.norm (/  i.new.norm  8))
                            (define %1.norm (+  i.new.norm  7))
                            (define %2.norm (extract  %1.norm i.new.norm Vx.norm))
                            (define %3.norm (extract  i.new.div.norm i.new.div.norm QuV.norm))
                            (define %4.norm (modulo  i.new.div.norm  4))
                            (define %5.norm (*  8  %4.norm))
                            (define %6.norm (+  %5.norm  7))
                            (define %7.norm (extract  %6.norm %5.norm Rt.norm))
                            (define %cond0.norm (bveq  %3.norm  (bv 1 1)))
                            (define %8.norm (if (equal? %cond0.norm #t) %7.norm (bv 0 8)))
                            (define %9.norm (bvor  %2.norm  %8.norm))
                            %9.norm
                            )
                  )
                )
      )
    )
  %dst0.norm
  )

(define (hexagon_V6_vrmpyub_128B  Vu Rt )
  (define Vd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 32))])
                (define Vd (bv #b0 32))
                (define %13.ext0.red
                  (apply
                    bvadd
                    (for/list ([iterator.0.new (reverse (range 0 32 8))])
                              (define %2 (+  iterator.0.new  i.new))
                              (define %3 (+  %2  7))
                              (define %4 (extract  %3 %2 Vu))
                              (define %6 (+  iterator.0.new  7))
                              (define %7 (extract  %6 iterator.0.new Rt))
                              (define %8 (zero-extend  %4 (bitvector 16)))
                              (define %9 (zero-extend  %7 (bitvector 16)))
                              (define %10 (bvmul  %8  %9))
                              (define %13 (zero-extend  %10 (bitvector 32)))
                              %13
                              )
                    ))
                (define %67.clone.2 (+  i.new  31))
                (define %13.acc0 (bvadd %13.ext0.red Vd))
                %13.acc0
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpybus_128B  Vu Rt )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define iterator.0.new.div (/  iterator.0.new  8))
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %3 (+  %2  7))
                            (define %4 (extract  %3 %2 Vu))
                            (define %5.new0 (/  i.new  16))
                            (define %6 (+  %5.new0  iterator.0.new.div))
                            (define %7 (modulo  %6  4))
                            (define %8 (*  8  %7))
                            (define %9 (+  %8  7))
                            (define %10 (extract  %9 %8 Rt))
                            (define %11 (zero-extend  %4 (bitvector 16)))
                            (define %12 (sign-extend  %10 (bitvector 16)))
                            (define %13 (bvmul  %11  %12))
                            %13
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vmpabus_128B  Vuu Rt )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 32))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define %1.new0 (/  i.new  2))
                            (define %3 (+  iterator.0.new  %1.new0))
                            (define %5 (+  %3  7))
                            (define %6 (extract  %5 %3 Vuu))
                            (define %7.new0 (*  iterator.0.new  2))
                            (define %8 (+  %7.new0  7))
                            (define %9 (extract  %8 %7.new0 Rt))
                            (define %10 (zero-extend  %6 (bitvector 16)))
                            (define %11 (sign-extend  %9 (bitvector 16)))
                            (define %12 (bvmul  %10  %11))
                            (define %17 (+  %3  1024))
                            (define %18 (+  %17  7))
                            (define %19 (extract  %18 %17 Vuu))
                            (define %64.new0 (/  iterator.0.new  4))
                            (define %65 (+  %64.new0  1))
                            (define %20 (*  8  %65))
                            (define %21 (+  %20  7))
                            (define %22 (extract  %21 %20 Rt))
                            (define %23 (zero-extend  %19 (bitvector 16)))
                            (define %24 (sign-extend  %22 (bitvector 16)))
                            (define %25 (bvmul  %23  %24))
                            (define %26 (bvadd  %12  %25))
                            %26
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vpackob_128B  Vu Vv %elemsize0 %elemsize1 %arg0 %arg2 %arg3 %arg5 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.new (reverse (range 0 512 %elemsize1))])
                              (define %8.new0 (*  %iterator1.new  2))
                              (define %9 (+  %arg3  %8.new0))
                              (define %lastidx3 (-  %elemsize1  1))
                              (define %10 (+  %9  %lastidx3))
                              (define %11 (extract  %10 %9 Vu))
                              %11
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new (reverse (range 0 512 %elemsize0))])
                              (define %1.new0 (*  %iterator0.new  2))
                              (define %2 (+  %arg0  %1.new0))
                              (define %lastidx1 (-  %elemsize0  1))
                              (define %3 (+  %2  %lastidx1))
                              (define %4 (extract  %3 %2 Vv))
                              %4
                              )
                    )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vaddcarrysat_128B  Vu Vv QsV )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i (reverse (range 0 32 1))])
                            (define %0 (*  32  i))
                            (define %1 (+  %0  31))
                            (define %2 (extract  %1 %0 Vu))
                            (define %5 (extract  %1 %0 Vv))
                            (define %6 (bvadd  %2  %5))
                            (define %7 (*  i  4))
                            (define %8 (extract  %7 %7 QsV))
                            (define %9 (sign-extend  %8 (bitvector 32)))
                            (define %10 (bvaddnsw  %6  %9 32))
                            %10
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vcombine_128B  Vu Vv )
  (define Vdd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (concat
                  (apply
                    concat
                    (for/list ([%iterator1.new (reverse (range 0 1024 8))])
                              (define %8 (+  %iterator1.new  7))
                              (define %9 (extract  %8 %iterator1.new Vu))
                              %9
                              )
                    )
                  (apply
                    concat
                    (for/list ([%iterator0.new (reverse (range 0 1024 8))])
                              (define %1 (+  %iterator0.new  7))
                              (define %2 (extract  %1 %iterator0.new Vv))
                              %2
                              )
                    )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vgtw_128B  %arg1.norm Vu.norm Vv.norm %arg0.norm %elemsize0.norm %arg2.norm )
  (define QdV.norm
    (apply
      concat
      (for/list ([%outer.it.norm (reverse (range 0 128 128))])
                (apply
                  concat
                  (for/list ([i.norm (reverse (range 0 128 %elemsize0.norm))])
                            (define %factor0.norm (/  %arg2.norm  %elemsize0.norm))
                            (define %3.new0.norm (*  i.norm  %factor0.norm))
                            (define %lastidx1.norm (-  %arg2.norm  1))
                            (define %4.norm (+  %3.new0.norm  %lastidx1.norm))
                            (define %5.norm (extract  %4.norm %3.new0.norm Vu.norm))
                            (define %9.norm (extract  %4.norm %3.new0.norm Vv.norm))
                            (define %10.norm (bvsgt  %5.norm  %9.norm))
                            (define %11.norm (if (equal? %10.norm #t) %arg0.norm %arg1.norm))
                            %11.norm
                            )
                  )
                )
      )
    )
  QdV.norm
  )

(define (hexagon_V6_vunpackub_128B  Vu %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx1))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (zero-extend  %2 (bitvector %arg0)))
                            %5
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vunpackh_128B  Vu %elemsize0 %arg0 )
  (define Vdd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1024 1024))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 1024 %elemsize0))])
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %1 (+  i.new  %lastidx1))
                            (define %2 (extract  %1 i.new Vu))
                            (define %5 (sign-extend  %2 (bitvector %arg0)))
                            %5
                            )
                  )
                )
      )
    )
  Vdd
  )

(define (hexagon_V6_vdmpybus_acc_128B  Vx Vu Rt )
  (define %dst0
    (apply
      concat
      (for/list ([i.new (reverse (range 0 1024 16))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 16 8))])
                            (define iterator.0.new.div (/  iterator.0.new  8))
                            (define %1 (+  i.new  15))
                            (define %2 (extract  %1 i.new Vx))
                            (define %5 (+  iterator.0.new  i.new))
                            (define %6 (+  %5  7))
                            (define %7 (extract  %6 %5 Vu))
                            (define %8.new1 (/  i.new  8))
                            (define %8.new0 (+  %8.new1  iterator.0.new.div))
                            (define %9 (modulo  %8.new0  4))
                            (define %10 (*  8  %9))
                            (define %11 (+  %10  7))
                            (define %12 (extract  %11 %10 Rt))
                            (define %13 (zero-extend  %7 (bitvector 16)))
                            (define %14 (sign-extend  %12 (bitvector 16)))
                            (define %15 (bvmul  %13  %14))
                            (define %16 (bvadd  %2  %15))
                            %16
                            )
                  )
                )
      )
    )
  %dst0
  )

(define (hexagon_V6_vsatwh_128B  Vu Vv )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 1 1))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 512 16))])
                            (define %0.new0 (*  i.new  2))
                            (define %1 (+  %0.new0  31))
                            (define %2 (extract  %1 %0.new0 Vv))
                            (define %3 (bvssat %2 32 16 ))
                            (define %10 (extract  %1 %0.new0 Vu))
                            (define %11 (bvssat %10 32 16 ))
                            (concat %11 %3)
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyiwb_128B  Vu Rt %innerlanesize0 %elemsize0 %arg0 %arg1 %arg2 %arg3 )
  (define Vd
    (apply
      concat
      (for/list ([%outer.it (reverse (range 0 32 32))])
                (apply
                  concat
                  (for/list ([i.new (reverse (range 0 %innerlanesize0 %elemsize0))])
                            (define i.new.div (/  i.new  %arg3))
                            (define %factor0 (/  %arg0  %elemsize0))
                            (define %0.new0 (*  i.new  %factor0))
                            (define %lastidx0 (-  %arg0  1))
                            (define %1 (+  %0.new0  %lastidx0))
                            (define %2 (extract  %1 %0.new0 Vu))
                            (define %3 (modulo  i.new.div  %arg2))
                            (define %4 (*  %elemsize0  %3))
                            (define %lastidx1 (-  %elemsize0  1))
                            (define %5 (+  %4  %lastidx1))
                            (define %6 (extract  %5 %4 Rt))
                            (define %8 (sign-extend  %2 (bitvector %arg1)))
                            (define %9 (sign-extend  %6 (bitvector %arg1)))
                            (define %10 (bvmul  %8  %9))
                            (define %13.high.idx (- %arg1 1))
                            (define %13.low.idx (+ (- %13.high.idx %arg0) 1) )
                            (define %13 (extract  %13.high.idx %13.low.idx %10))
                            %13
                            )
                  )
                )
      )
    )
  Vd
  )

(define (hexagon_V6_vmpyuh_128B  Vu Rt )
  (define Vdd
    (apply
      concat
      (for/list ([i.new (reverse (range 0 2048 64))])
                (apply
                  concat
                  (for/list ([iterator.0.new (reverse (range 0 32 16))])
                            (define %1.new0 (/  i.new  2))
                            (define %2 (+  iterator.0.new  %1.new0))
                            (define %3 (+  %2  15))
                            (define %4 (extract  %3 %2 Vu))
                            (define %6 (+  iterator.0.new  15))
                            (define %7 (extract  %6 iterator.0.new Rt))
                            (define %8 (zero-extend  %4 (bitvector 32)))
                            (define %9 (zero-extend  %7 (bitvector 32)))
                            (define %10 (bvmul  %8  %9))
                            %10
                            )
                  )
                )
      )
    )
  Vdd
  )


;; ================================================================================

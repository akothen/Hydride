...
(define (vec-len expr)
  (destruct expr
    ...
    [(buffer-index index elemT buffsize) (/ buffsize (intr-elemT-size elemT)) ]
...
(define (interpret-env p env)
  (destruct p
    ...
    [(buffer-index index elemT buffsize) (lambda (i) (buffer-ref (create-buffer (vector-ref env index) elemT) i))]
    [(xBroadcast sca factor) 
            (lambda (i) ((interpret-env sca env) 0))
     ]
    [(int-imm data signed?) 
     
            (lambda (i) (imm-ref data signed?) )]
    [(vec-sat-add v1 v2) (lambda (i) (do-sat-add ((interpret-env v1 env) i) ((interpret-env v2 env) i)))]
    [(vec-sat-sub v1 v2) (lambda (i) (do-sat-sub ((interpret-env v1 env) i) ((interpret-env v2 env) i)))]
...
(define (do-rounding-halving-add lhs rhs round?)
  ...
  (define  one
    (cond
      [round?
        (bv 1 (bitvector widened-size))
        ]
      [else
        (bv 0 (bitvector widened-size))
        ]
      )
    )
  (define result 

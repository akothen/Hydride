...
(define (is-constant-expr expr)
  (destruct expr [(xBroadcast sca factor) (is-constant-expr sca)] [(int-imm data signed?) #t] [_ #f]))
...
(define (is-simple-ramp ramp-op)
  (destruct ramp-op
            [(ramp base stride len) (destruct stride [(int-imm data signed?) #t] [_ #f])]
            [_ #f]))
...
(define (get-bv-ops p)
  (destruct
   ...
   [(ramp base stride len)
    (append (list extract bvadd 'ramp)
            (if (is-simple-ramp (ramp base stride len))
                (list)
   ...
   [(cast-int vec olane oprec)
    (cond
      ;; Upcast
      [(> (vec-size p) (vec-size vec)) (append signed-upcasting-list (get-bv-ops vec))]
      ;; Downcast
      [(< (vec-size p) (vec-size vec)) (append signed-downcasting-list (get-bv-ops vec))]
      ;; Same length -> reinterpret cast between signedness
      [else (append signed-casting-list (get-bv-ops vec))])]
   [(cast-uint vec olane oprec)
    (cond
      ;; Upcast
      [(> (vec-size p) (vec-size vec)) (append unsigned-upcasting-list (get-bv-ops vec))]
      ;; Downcast
      [(< (vec-size p) (vec-size vec)) (append unsigned-downcasting-list (get-bv-ops vec))]
      ;; Same length -> reinterpret cast between signedness
      [else (append unsigned-casting-list (get-bv-ops vec))])]
   [(vec-saturate vec olane oprec signed?)
    (cond
      [signed? (append (list bvssat sign-extend extract) (get-bv-ops vec))]
      [else (append (list bvusat zero-extend extract) (get-bv-ops vec))])]
   ...
   ;; Operations
   [(sca-add v1 v2)
    (append (list extract bvadd)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend)
                (list zero-extend))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   [(sca-sub v1 v2)
    (append (list extract bvsub)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend)
                (list zero-extend))
            (get-bv-ops v1)
   ...
   [(sca-div v1 v2)
    (append (list extract)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvsdiv bvashr)
                (list zero-extend bvudiv bvlshr))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   [(sca-mod v1 v2)
    (append (list extract)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvsrem bvsmod)
                (list zero-extend bvurem bvurem))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   [(sca-min v1 v2)
    (append (list extract)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvsmin)
                (list zero-extend bvumin))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   [(sca-max v1 v2)
    (append (list extract)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvsmax)
                (list zero-extend bvumax))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   ...
   [(sca-lt v1 v2)
    (append (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvslt)
                (list zero-extend bvult))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   [(sca-le v1 v2)
    (append (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvsle)
                (list zero-extend bvule))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   ...
   [(sca-absd v1 v2)
    (append (list bvsub)
            (if (halide:is-signed-expr? v1 v2)
                (list bvsge bvsmax bvsmin)
                (list bvuge bvumax bvumin))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   [(sca-shl v1 v2)
    (append (list bvshl)
   ...
   [(sca-shr v1 v2)
    (append (if (halide:is-signed-expr? v1 v2)
                (list bvashr sign-extend)
                (list bvlshr zero-extend))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   ...
   [(vec-sat-sub v1 v2)
    (append (list extract)
            (if (halide:is-signed-expr? v1 v2)
                (list bvsubnsw 'bvssat)
                (list bvsubnuw 'bvusat))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   ...
   [(vec-rounding_mul_shift_right v1 v2 v3)
    (append (list extract bvmul sign-extend zero-extend)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvashr bvssat bvsdiv)
                (list zero-extend bvlshr bvusat bvudiv))
            (get-bv-ops v1)
            (get-bv-ops v2)
            (get-bv-ops v3))]
   [(vec-mul_shift_right v1 v2 v3)
    (append (list extract bvmul sign-extend zero-extend)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvashr bvssat)
                (list zero-extend bvlshr bvusat))
            (get-bv-ops v1)
            (get-bv-ops v2)
            (get-bv-ops v3))]
   [(vec-rounding_shift_right v1 v2)
    (append (list extract bvshl)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvashr bvssat bvsdiv)
                (list zero-extend bvlshr bvusat bvudiv))
            (get-bv-ops v1)
   ...
   [(vec-rounding_halving_add v1 v2)
    (append (list extract bvadd)
            (if (halide:is-signed-expr? v1 v2)
                (list sign-extend bvsdiv)
                (list zero-extend bvudiv))
            (get-bv-ops v1)
            (get-bv-ops v2))]
   ...
   [(vector_reduce op width vec)
    (cond
      [(eq? op 'add)
       (append (list extract bvadd)
               (if (halide:is-signed-expr? vec vec)
                   (list sign-extend)
                   (list zero-extend))
               (get-bv-ops vec))]
      [else (error "Unexpected vector_reduce op:" op)])]
   [(vec-tile_matmul m n k v0 v1 v2)
    (append (list extract bvadd bvmul sign-extend zero-extend)
            (get-bv-ops v0)
            (get-bv-ops v1)
            (get-bv-ops v2))]
   ;; Shuffles
  ...
  (define (get-latest-arg val)
    (cond
      ...
      [(and (is-non-const-expr val) (xBroadcast? val))
       ...
       (destruct val [(xBroadcast bval factor) (xBroadcast (arg (- latest-arg 1)) factor)])]
      [else val]))
  ...
  (cond
    [(is-non-const-expr expr)
     (destruct
      expr
      ;; Constructors
      [(x8 sca) (broadcast-helper x8 sca)]
      [(x16 sca) (broadcast-helper x16 sca)]
      [(x32 sca) (broadcast-helper x32 sca)]
      [(x64 sca) (broadcast-helper x64 sca)]
      [(x128 sca) (broadcast-helper x128 sca)]
      [(x256 sca) (broadcast-helper x256 sca)]
      [(x512 sca) (broadcast-helper x512 sca)]
      ...
      ;; Operations
      [(vec-add v1 v2) (binary-op-helper vec-add v1 v2)]
      [(vec-sat-add v1 v2) (binary-op-helper vec-sat-add v1 v2)]
      [(vec-sub v1 v2) (binary-op-helper vec-sub v1 v2)]
      [(vec-sat-sub v1 v2) (binary-op-helper vec-sat-sub v1 v2)]
      [(vec-mul v1 v2) (binary-op-helper vec-mul v1 v2)]
      [(vec-widen-mul v1 v2) (binary-op-helper vec-widen-mul v1 v2)]
      ;[(vec-rounding_shift_right v1 v2)
      ;(if is-leaf-depth
      ...
      ;  )
      ;]
      [(vec-rounding_shift_right v1 v2) (binary-op-helper vec-rounding_shift_right v1 v2)]
      [(vec-rounding_halving_add v1 v2) (binary-op-helper vec-rounding_halving_add v1 v2)]
      [(vec-halving_add v1 v2) (binary-op-helper vec-halving_add v1 v2)]
      ;[(vec-rounding_mul_shift_right v1 v2 v3)
      ;(if is-leaf-depth
      ...
      [(vec-div v1 v2) (binary-op-helper vec-div v1 v2)]
      [(vec-mod v1 v2) (binary-op-helper vec-mod v1 v2)]
      [(vec-min v1 v2) (binary-op-helper vec-min v1 v2)]
      [(vec-max v1 v2) (binary-op-helper vec-max v1 v2)]
      [(vec-if v1 v2 v3)
       (if is-leaf-depth
      ...
      [(vec-eq v1 v2) (binary-op-helper vec-eq v1 v2)]
      [(vec-lt v1 v2) (binary-op-helper vec-lt v1 v2)]
      [(vec-le v1 v2) (binary-op-helper vec-le v1 v2)]
      [(vec-abs v1)
       (if is-leaf-depth
      ...
      [(vec-absd v1 v2) (binary-op-helper vec-absd v1 v2)]
      [(vec-shl v1 v2) (binary-op-helper vec-shl v1 v2)]
      [(vec-shr v1 v2) (binary-op-helper vec-shr v1 v2)]
      [(vec-bwand v1 v2) (binary-op-helper vec-bwand v1 v2)]
      [(vec-bwnot v1)
       (if is-leaf-depth
      ...
      [(vec-tile_matmul m n k v0 v1 v2)
       (if is-leaf-depth
           (values (vec-tile_matmul m n k (arg 0) (arg 1) (arg 2)) 3)
           (begin
             (define-values (leaf1-sol args-used1) (bind-expr-args v0 args (- depth 1)))
             (define remaining-values1 (- (vector-length args) args-used1))
             (define remaining-args1 (vector-take-right args remaining-values1))

             (define-values (leaf2-sol args-used2) (bind-expr-args v1 remaining-args1 (- depth 1)))
             (define remaining-values2 (- (vector-length remaining-args1) args-used2))
             (define remaining-args2 (vector-take-right remaining-args1 remaining-values2))

             (define-values (leaf3-sol args-used3) (bind-expr-args v2 remaining-args2 (- depth 1)))

             (values (vec-tile_matmul m n k leaf1-sol leaf2-sol leaf3-sol)
                     (+ args-used1 args-used2 args-used3))))]
      ;; Shuffles
      [(vec-broadcast n vec)
       (if is-leaf-depth
      ...
      [(interleave4 v1 v2 v3 v4)
       ...
       (if is-leaf-depth
           ...
           (begin
             ...
             (values (interleave4 leaf1-sol leaf2-sol leaf3-sol leaf4-sol)
                     (+ args-used1 args-used2 args-used3 args-used4))))]
...
(define (get-sub-exprs expr depth)
  ...
  (cond
    ...
    [(and (xBroadcast? expr) (equal? depth 1)) (get-sub-exprs expr 2)]
    [(<= depth 0) (list)]

    ;; If it's a broadcast we should include it for more simplification
    [(equal? depth 1) (list expr)]
    [else
...
(define (vec-size expr)
  (destruct
   ...
   [(vec-tile_matmul m n k v0 v1 v2) (vec-size v0)]
   ;; Shuffles
   ...
   ;; Base case
   [_
    (error "halide\\ir\\interpreter.rkt: Don't know how to infer vector size for Halide expression:"
           expr)]))
...
(define (scale-down-expr expr scale-factor)
  ...
  (define (visitor-fn e)
    (destruct e
              ...
              [(xBroadcast sca factor) (xBroadcast sca (/ factor scale-factor))]
              [(vec-broadcast n vec) (vec-broadcast (/ n scale-factor) vec)]
              [(x128 sca)
               (cond
                 [(equal? scale-factor 2) (x64 sca)]
                 [(equal? scale-factor 4) (x32 sca)]
                 [(equal? scale-factor 8) (x16 sca)]
...
(define (contains-complex-op-arm expr)
  ...
  (define flag #f)
  (define (visitor-fn e)
    (destruct e [(vec-shr v1 v2) (set! flag #t)] [_ e]))
  (halide:visit expr visitor-fn)
  ...
  (define (visitor-fn e)
    (destruct e
              [(ramp base stride len)
               ;(set! flag #t)
               e]
              [(vec-div v1 v2) (set! flag #t)]
              [(vec-if vi v1 v2) (set! flag #t)]
              [(vec-rounding_mul_shift_right v1 v2 v3) (set! flag #t)]
              [_ e]))
  (halide:visit expr visitor-fn)
...
(define (contains-mul-op expr)
  ...
  (define flag #f)
  (define (visitor-fn e)
    (destruct e [(vec-mul v1 v2) (set! flag #t) e] [(vec-widen-mul v1 v2) (set! flag #t) e] [_ e]))
  (halide:visit expr visitor-fn)
    ...
    [else
     ...
     (define sub-results
       (for/list ([leaf leaves])
         (extract-sub-expressions leaf depth)))
...
(define (get-imm-values expr)
  ...
  (define (visitor-fn e)
    (destruct
     e
     [(int-imm data signed?) (set! imms-vals (append imms-vals (list data))) e]
     [(vector_reduce op width vec)
     ...
     [(vec-shr v1 v2)
      ...
      (define adjusted
        (for/list ([val shift-imms])
          (if (< (bvlength val) 32)
              (zero-extend val (bitvector 32))
              val)))
     ...
     [(vec-saturate v1 olane oprec signed?)
      ...
      (define-values (max-val min-val)
        (cond
          [signed? (values (bvsmaxval prec) (bvsminval prec))]
          [else (values (bvumaxval prec) (bvuminval prec))]))
      e]
...
(define (get-widened-elemT elemT1 elemT2)
  (cond
    ...
    [else (error "unsupported type for widening in halide" elemT1 elemT2)]))
...
(define (get-elemT expr)
  (destruct
   ...
   [(vec-tile_matmul m n k v0 v1 v2) (get-elemT v0)]
   ;; Shuffles
...
(define (vec-precision expr)
  (destruct
   ...
   [(vec-tile_matmul n m k v0 v1 v2) (vec-precision v0)]
   ;; Shuffles
   ...
   ;; Base case
   [_
    (error "halide\\utils.rkt: Don't know how to infer vector precision for Halide expression:"
           expr)]))
...
(define (is-buffer-hash e)
  (cond
    [(equal? e '(buf uint8 32)) (values #t 'uint8 32)]
    [(equal? e '(buf uint8 64)) (values #t 'uint8 64)]
    [(equal? e '(buf uint8 128)) (values #t 'uint8 128)]
    [(equal? e '(buf uint8 256)) (values #t 'uint8 256)]
    [(equal? e '(buf uint8 512)) (values #t 'uint8 512)]
    [(equal? e '(buf uint8 1024)) (values #t 'uint8 1024)]
    [(equal? e '(buf uint8 2048)) (values #t 'uint8 2048)]
    [(equal? e '(buf int8 32)) (values #t 'int8 32)]
    [(equal? e '(buf int8 64)) (values #t 'int8 64)]
    [(equal? e '(buf int8 128)) (values #t 'int8 128)]
    [(equal? e '(buf int8 256)) (values #t 'int8 256)]
    [(equal? e '(buf int8 512)) (values #t 'int8 512)]
    [(equal? e '(buf int8 1024)) (values #t 'int8 1024)]
    [(equal? e '(buf int8 2048)) (values #t 'int8 2048)]
    [(equal? e '(buf uint16 32)) (values #t 'uint16 32)]
    [(equal? e '(buf uint16 64)) (values #t 'uint16 64)]
    [(equal? e '(buf uint16 128)) (values #t 'uint16 128)]
    [(equal? e '(buf uint16 256)) (values #t 'uint16 256)]
    [(equal? e '(buf uint16 512)) (values #t 'uint16 512)]
    [(equal? e '(buf uint16 1024)) (values #t 'uint16 1024)]
    [(equal? e '(buf uint16 2048)) (values #t 'uint16 2048)]
    [(equal? e '(buf int16 32)) (values #t 'int16 32)]
    [(equal? e '(buf int16 64)) (values #t 'int16 64)]
    [(equal? e '(buf int16 128)) (values #t 'int16 128)]
    [(equal? e '(buf int16 256)) (values #t 'int16 256)]
    [(equal? e '(buf int16 512)) (values #t 'int16 512)]
    [(equal? e '(buf int16 1024)) (values #t 'int16 1024)]
    [(equal? e '(buf int16 2048)) (values #t 'int16 2048)]
    [(equal? e '(buf uint32 32)) (values #t 'uint32 32)]
    [(equal? e '(buf uint32 64)) (values #t 'uint32 64)]
    [(equal? e '(buf uint32 1024)) (values #t 'uint32 1024)]
    [(equal? e '(buf uint32 2048)) (values #t 'uint32 2048)]
    [(equal? e '(buf int32 32)) (values #t 'int32 32)]
    [(equal? e '(buf int32 64)) (values #t 'int32 64)]
    [(equal? e '(buf int32 1024)) (values #t 'int32 1024)]
    [(equal? e '(buf int32 2048)) (values #t 'int32 2048)]
    [else (values #f 0 0)]))

(define (get-halide-expr-regs expr)
  (define-values (is-buf? type size) (is-buffer-hash expr))
  (cond
    [is-buf? (list (halide:create-buffer (bv 0 size) type))]

    [(buffer? expr) (list expr)]
    [else
     (define sub-exprs (halide:sub-exprs expr))
     (define nested-regs
       (for/list ([sub-exp sub-exprs])
         (get-halide-expr-regs sub-exp)))
     ...
     (cond
       [(not (equal? (length intersect-types) 0))
        ...
        (cond
          ...
          [(and (equal? sizes-e1 sizes-e2)) (verify-equivalent-halide-exprs e1 e2)]
...
(define (verify-equivalent-halide-exprs e1 e2)
  ...
  (define-values (symbolic-e1 num-used-1)
    (bind-expr-args dummy-e1 (list->vector bound-regs-1) (+ depth-e1 1)))
  ...
  (define (equiv-at-index i)
    ...
    (clear-vc!)
    (define sol
      (synthesize #:forall (list sym-bvs) #:guarantee (assert (equal? e1-result-i e2-result-i))))
  ...
  (cond
    [equiv
     (define e1-result
       (halide:assemble-bitvector (halide:interpret symbolic-e1) (halide:vec-len symbolic-e1)))
     (define e2-result
       (halide:assemble-bitvector (halide:interpret symbolic-e2) (halide:vec-len symbolic-e2)))

     (clear-vc!)
     (define sol
       (synthesize #:forall (list sym-bvs) #:guarantee (assert (equal? e1-result e2-result))))
...
(define (get-common-expressions expr-ls-1 expr-ls-2)
  ...
  (for/list ([e1 exprs-1])
    (for/list ([e2 exprs-2])
      ...
      (cond
        [equivalent? (set! common-expressions (append common-expressions (list (list e1 e2))))])))
  common-expressions)

(define (is-int-min? bits broadcast-expr signed?)
  (define int-min-value
    (cond
      [signed? (bvsminval bits)]
      [else (bvuminval bits)]))

  (define cvt-to-integer
    (cond
      [signed? bitvector->integer]
      [else bitvector->natural]))
...
(define (match-saturating-cast expr)
  (define (visitor-fn ele)
    (match ele
      [(cast-int (vec-max (vec-min min1 min2) max2) olanes prec)

       (cond
         [(and (is-int-min? prec max2 #t) (is-int-max? prec min2 #t))
          (vec-saturate min1 olanes prec #t)]
         [else ele])]
      [(cast-uint (vec-max (vec-min min1 min2) max2) olanes prec)
       ...
       (cond
         ...
         [else ele])]
      [(vec-mul (cast-int reg_0 lanes prec) (xBroadcast (int-imm lit sign) lanes))
       ...
       (cond
         [(and (equal? sign #t)
          ...
          (vec-widen-mul reg_0
                         (xBroadcast (int-imm (bv (bitvector->integer lit) (bitvector (/ prec 2))) #t)
                                     (* lanes 1)))]
         [else ele])]
      [(vec-mul (cast-uint reg_0 lanes prec) (xBroadcast (int-imm lit sign) lanes))
       ...
       (cond
         [(and (equal? sign #f)
          ...
          (vec-widen-mul reg_0
                         (xBroadcast (int-imm (bv (bitvector->natural lit) (bitvector (/ prec 2))) #f)
                                     (* lanes 1)))]
         [else ele])]
...
(define (cvt-hashed-expr-to-actual-expr hashed-expr)
  ...
  (define (raise-fn ele)
    ...
    (cond
      [is-buf? (halide:create-buffer (bv 0 size) type)]
      [else ele]))
  ...
  (for/list ([expr input-expr-ls])
    ...
    (cond
      [(hash-has-key? hashed-log expr)
       ...
       (define synthesizable? (vector-ref hashed-entry 0))
  ...
  (define (replace-arg e)
    (destruct e
              [(reg id)
               ...
               (halide:create-buffer (bv 0 (list-ref sizes index))
                                     (size-to-elemT-signed (list-ref precs index)
                                                           (list-ref signedness index)))]
              [_ e]))

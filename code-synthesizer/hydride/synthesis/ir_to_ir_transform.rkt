...
(define (inst-combine spec-expr optimize? symbolic? solver input-sizes input-precs target-language)
  ...
  (define language-type
    (cond
      [(equal? target-language "hvx") 'hvx]
      [(equal? target-language "arm") 'arm]
      [(equal? target-language "x86") 'x86]))

  (rewrite-ir spec-expr
              1
              4
              optimize?
              symbolic?
...
(define (rewrite-ir spec-expr
  ...
  (define-values (src-interpreter src-cost-fn src-visitor src-length-fn src-prec-fn src-get-ops)
    (cond
      [(equal? src-language 'x86)
       (values hydride:interpret
               hydride:cost
               hydride:visitor
               hydride:get-length
               hydride:get-prec
      ...
      [else (error "Unsupported src language in rewrite-ir" src-language)]))

  (define-values (target-interpreter target-cost-fn
                                     target-visitor
                                     target-length-fn
                                     target-prec-fn
                                     target-get-ops)
    (cond
      ...
      [(equal? target-language 'arm)
       (set-target-arm)
       (values arm:interpret arm:cost arm:visitor arm:get-length arm:get-prec arm:get-bv-ops)]
      [else (error "Unsupported target language in rewrite-ir" target-language)]))
                         ...
                         (cond
                           [(not solved?)
                            ...
                            (define-values (grammar interpreter cost-model)
                              (get-expr-grammar-step-hydride spec-expr
                                                             base_name
                                                             src-get-ops
                                                             src-visitor
                                                             src-length-fn

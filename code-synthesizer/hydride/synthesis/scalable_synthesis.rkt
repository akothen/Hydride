...
(define (scale-down-synthesis halide-expr
  ...
  (define (verify-upscaled-equal? hydride-expr)
    ...
    (define-values (expr-extract num-used)
      (halide:bind-expr-args halide-expr halide-expr-args actual-expr-depth))

    (debug-log expr-extract)

    (define halide-res
      (halide:assemble-bitvector (halide:interpret expr-extract) (halide:vec-len expr-extract)))
    (define hydride-res
      (cond
    ...
    (define cex (verify (assert (equal? halide-res hydride-res))))
    ...
    (destruct
     ...
     [_
      (begin
        ...
        (define-values (_ scaled-down-expr)
          (halide:scale-down-expr halide-expr effective-scale-factor))
        (define-values (expr-extract num-used)
          (halide:bind-expr-args scaled-down-expr dummy-args actual-expr-depth))
        ...
        (define (invoke-spec env-full)
          ...
          (define-values (_expr-extract-full _num-used)
            (halide:bind-expr-args scaled-down-expr-full synth-buffers-full actual-expr-depth))
        ...
        (define (invoke-spec-lane lane-idx env-lane)
          ...
          (define-values (_expr-extract _num-used)
            (halide:bind-expr-args scaled-down-expr-lane synth-buffers-lane actual-expr-depth))
        ...
        (if satisfiable?
            ...
            (begin
              ...
              (define-values (expr-extract num-used)
                (halide:bind-expr-args halide-expr recalculate-args actual-expr-depth))
              (define expr-VF (halide:vec-len expr-extract))
                     ...
                     [(not solved?)
                      ...
                      (cond
                        ...
                        [(and sat? (<= (cost-model mat) min-cost))
                         (set! sol mat)
                         (set! min-cost (cost-model mat))])]))) ; (thread thunk
                 ) ;; paramterize
               ) ;; threads list
             ) ;; thds

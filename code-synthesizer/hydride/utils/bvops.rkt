...
(define (bvaddnsw a b bitwidth)
  ...
  (define result
    (cond
      [(and  (bvsgt a zero)  (bvsgt b zero) (bvsle c zero) )
       (bvsmaxval bitwidth) ]
      [(and (bvslt a zero) (bvslt b zero) (bvsge c zero))
       (bvsminval bitwidth)]
      [else (bvadd a b)]
      )
    )
...
(define (bvsubnsw a b bitwidth)
  ...
  (define result
    (cond
      [(and (bvsgt b zero)
            (bvslt a  (bvadd (bvsminval bitwidth) b)))
       (bvsminval bitwidth)]
      [(and (bvslt b zero)
            (bvsgt a (bvadd (bvsmaxval bitwidth) b))
            )
       (bvsmaxval bitwidth)]
      [else (bvsub a b)]
      )
...
(define (bvmulnsw a b bitwidth)
  ...
  (define result
    (cond
      ...
      [(bveq a minusonebv)
       (cond [(bveq b (bvsminval bitwidth)) (bvsmaxval bitwidth)]
             [else (bvmul a b)])
       ]
      [(bveq b minusonebv)
       (cond [(bveq a (bvsminval bitwidth)) (bvsmaxval bitwidth)]
             [else (bvmul a b)])
       ]
      [(bvsgt b (bv 0 bitwidth))
       (cond [ (bvsgt a (bvsdiv (bvsmaxval bitwidth) b))
               (bvsmaxval bitwidth)]
             [ (bvslt a (bvsdiv (bvsminval bitwidth) b))
               (bvsminval bitwidth)]
             [else (bvmul a b)])
       ]
      [(bvslt b (bv 0 bitwidth))
       (cond [ (bvslt a (bvsdiv (bvsmaxval bitwidth) b))
               (bvsmaxval bitwidth)]
             [ (bvsgt a (bvsdiv (bvsminval bitwidth) b))
               (bvsminval bitwidth)]
             [else (bvmul a b)])
       ]

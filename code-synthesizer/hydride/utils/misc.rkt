...
(define (swizzle idx group_size dis_size rot_factor)
  ...
  (define result
    (group group_size
           (rotate rot_factor group_size (displace dis_size group_size (modulo idx group_size)))
           idx))
...
;;                                        8         16         0          16         8           8       0
(define (vector-two-input-swizzle v1
                                  v2
                                  num_elems
                                  type_size
                                  lane_offset
    ...
    (apply
     ...
     (for/list ([i
                 (range
                  0
                  num_elems
                  lane_size)]) ;; 0 -> 8 step 16 -> only runs once hence min(1, num_elems / lane_size)
       (apply
        concat
        (for/list ([j (range
                       lane_offset
                       (* 2
                          high_lane_offset))]) ;; 0 -> 16 step one -> (2 * (group_size + lane_offset))
          ...
          (if (< swizzled-index group_size)
              ...
              (begin
                (define adjusted-swizzled-index
                  (- (- num_elems 1) (- (+ i lane_offset swizzled-index) group_size)))
...
(define (assume-signedness sym-bv prec signed?)
  ...
  (for/list ([i (range (/ bv-size prec))])
    ...
    (cond
      [signed? '()]
      [(concrete? sym-bv) '() ;; No assumptions needed for concrete values
       ]
      [else

...
class RoseFunction(RoseValue, RoseRegion):
        ...
        if not isinstance(ReturnValue, RoseOperation) \
            ...
            if Suffix != "":
                ClonedReturnVal = ReturnValue.clone(
                    ReturnValue.getName() + "." + Suffix, ChangeID)
            else:
    ...
    def eraseArg(self, ArgIndex: int):
        ...
        FunctionType = RoseFunctionType.create(
            ArgTyList, self.RetVal.getType())
    ...
    def appendArg(self, NewArg: RoseValue):
        ...
        Arg = RoseArgument.create(
            NewArg.getName(), NewArg.getType(), RoseUndefValue())
    ...
    def setRetVal(self, Value):
        ...
        NewFunctionType = RoseFunctionType.create(
            self.getType().getArgList(), RetType)
...
class RoseForLoop(RoseRegion):
    def __init__(self, IteratorName: str, Start: RoseValue, End: RoseValue, Step: RoseValue,
                 RegionList: list, ParentRegion: RoseRegion):
        self.Iterator = RoseValue.create(
            IteratorName, RoseIntegerType.create(32))
        self.Start = Start
        self.End = End
        self.Step = Step
    ...
    def create(*args):
        if len(args) == 6:
            ...
            if isinstance(args[0], str) and isinstance(args[1], int) \
                    ...
                    and isinstance(args[4], list) and isinstance(args[5], RoseRegion):
                Start = RoseConstant. create(
                    args[1], RoseIntegerType.create(32))
                End = RoseConstant. create(args[2], RoseIntegerType.create(32))
                Step = RoseConstant. create(
                    args[3], RoseIntegerType.create(32))
    ...
    def clone(self, Suffix: str = "", ValueToValueMap: dict = dict(),  ChangeID: bool = False):
        ...
        ClonedLoop = RoseForLoop.create(
            IteratorName, LoopStart, LoopEnd, LoopStep)
...
class RoseCond(RoseRegion):
    ...
    def clone(self, Suffix: str = "", ValueToValueMap: dict = dict(), ChangeID: bool = False):
        ...
        ClonedCondRegion = RoseCond.create(
            ClonedConditions, len(self.getKeys()))
        for Abstraction in self:
            ClonedAbstraction = Abstraction.clone(
                Suffix, ValueToValueMap, ChangeID)

...
class x86CIntrinsicsWrappersEmitter():
    def __init__(self):
        ...
        FunctionInfoList = CodeGenerator.codeGen(
            ExtractConstants=False, JustGenRosette=False)
    ...
    def generateWrappers(self):
        ...
        for FunctionInfo in self.FunctionInfoList:
            ...
            Content += self.generateWrapperForInstuction(
                TestDirName, FunctionInfo)
    ...
    def genCWrapperFunction(self, FunctionInfo: RoseFunctionInfo):
        ...
        for Index, Param in enumerate(Function.getArgs()):
            ...
            if Sema.params[Index].is_imm == False:
                ...
                TypedParams.append(RoseARMCCodeEmitter.ARMToC(
                    Sema.params[Index].type) + " " + Param.getName())
            else:
        ...
        if ImmWidth == None:
            ...
            InvokedFunction = "{}({})".format(
                Function.getName(), ", ".join(ParamsNames))
        ...
        for Idx in range(lo, hi+1):
            ...
            Content.append(FuncSig)
            InvokedFunction = "{}({}, {})".format(
                Function.getName(), ", ".join(ParamsNames), Idx)
            Content.append("  return " + InvokedFunction + ";")
            Content.append("}\n\n")
        return "\n".join(Content)

...
(define (sobel_x v1 v2)
  ...
  (define (bvabs x)
    (if
      (bvsge x (bv 0 (bitvector (bvlength x))))
      x
      (bvmul (bv -1 (bitvector (bvlength x))) x)
      )
    )
...
(define (invoke_add_output env)
  (add_output (vector-ref env 0)
         (vector-ref env 1)
         )
  )

(define (invoke_clamp env)
  (clamp (vector-ref env 0)
         )
  )
...
(define (add_output_grammar_mem vars #:depth k)
	(assert (> k 0))
	(cond
	[(choose* #t #f) (apply choose* vars)]
	[else
...
(define (add_output_grammar_shuffle vars #:depth k)
	(assert (> k 0))
	(cond
	[(choose* #t #f) (apply choose* vars)]
	[(choose* #t #f)
	(
		vector-two-input-swizzle_dsl
		(add_output_grammar_shuffle vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		(add_output_grammar_shuffle vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		8				;; Integer Operand 
		16				;; Precision Operand 
		0				;; Integer Operand 
	...
	[else
	(add_output_grammar_mem vars #:depth k)
	]
	)
)

(define (add_output_grammar_operations vars #:depth k)
	(assert (> k 0))
	(cond
	[(choose* #t #f) (apply choose* vars)]
	[(choose* #t #f)
	(
		_m_paddw_dsl
		(add_output_grammar_operations vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		(add_output_grammar_operations vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		128				;; Integer Operand 
		128				;; Lane Size 
		128				;; Integer Operand 
	...
	[else
	(add_output_grammar_shuffle vars #:depth k)
	]
	)
)
...
(define (clamp_grammar_mem vars #:depth k)
	(assert (> k 0))
	(cond
	[(choose* #t #f) (apply choose* vars)]
	[else
...
(define (clamp_grammar_shuffle vars #:depth k)
	(assert (> k 0))
	(cond
	[(choose* #t #f) (apply choose* vars)]
	[(choose* #t #f)
	(
		vector-two-input-swizzle_dsl
		(clamp_grammar_shuffle vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		(clamp_grammar_shuffle vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		8				;; Integer Operand 
		16				;; Precision Operand 
		0				;; Integer Operand 
	...
	[else
	(clamp_grammar_mem vars #:depth k)
	]
	)
)

(define (clamp_grammar_operations vars #:depth k)
	(assert (> k 0))
	(cond
	[(choose* #t #f) (apply choose* vars)]
	[(choose* #t #f)
	(
		_mm256_max_epi8_dsl
		(clamp_grammar_operations vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		(clamp_grammar_operations vars #:depth (- k 1)) ;; 128-bit Bitvector operand
		128				;; Integer Operand 
		128				;; Lane Size 
		128				;; Integer Operand 
	...
	[else
	(clamp_grammar_shuffle vars #:depth k)
	]
	)
)
...
(define grammar-fns 
  (list
    ...
    (lambda (i) (add_output_grammar_operations (list (reg 0) (reg 1)) #:depth i  )  )

    (lambda (i) (clamp_grammar_operations (list (reg 0) (reg 1)) #:depth i  )  )
...
(for/list ([i (range (length spec_exprs))])
          ...
          (define-values (satisfiable? materialize elap) 
                         (synthesize-sol-with-depth 2 depth-limit sub_expr test_grammar-fn bw-list optimize? cost symbolic?) )

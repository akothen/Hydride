...
(clear-vc!)
(define (synth_check arg0 num_rows num_cols idx-i idx-j )
  (define input_size (* num_rows num_cols 8))
  (vector-div 
    (dsl_inst_1 
...
(define (wrapper arg0 num_rows num_cols) 
  (apply concat (for/list ([i (range (+ (- num_rows 3) 1))])
                          (apply concat (for/list ([j (+ (- num_cols 3) 1)])
                                                  (synth_check arg0 num_rows num_cols i j )
                                                  )))))




;; Synthesis result was original created for 6x6 tensor, hence 
;; Choosing row and col should be >= 6
(define test-scale-row 64)
(define test-scale-col 64)



(define large-image (apply concat (for/list ([k (range (* test-scale-row test-scale-col))])
                                            (bv (+ k 1) (bitvector 8))
                                            )))

(displayln "Box-Blur Window on Large inputs")
(print-mat (box-blur large-image test-scale-row test-scale-col 3 3 8) (- test-scale-row 2) (- test-scale-col 2) 8)


(displayln "Synth_Res on Large inputs")
(print-mat (wrapper large-image test-scale-row test-scale-col ) (- test-scale-row 2) (- test-scale-col 2) 8)

(define-symbolic sym_arg0 (bitvector (* 8 test-scale-row test-scale-col)))
(define spec-res (box-blur sym_arg0 test-scale-row test-scale-col 3 3 8))
(define cex (verify (begin 
                      (assert (equal? (wrapper sym_arg0 test-scale-row test-scale-col)   spec-res)    )
                      ) )

...
(define (ra16_lane_sel lane_number total_num_lanes xstart xoffsets xoffsets_hi xsquare)
  ...
  (define perm_idx
    (if (even? lane_number)
        ...
        (modulo (+ lane_number
                   (+ xstart
                      (* 2 (ra16_aux lane_number 16 bvxoffsets))
                      (* 2 (+ 1 (ra16_aux (- lane_number 1) 16 bvxoffsets)))))
                total_num_lanes)))
  perm_idx)
...
(define (mul_conv matA matB m n indatasize outdatasize) 
  (define dst
    (apply concat
      (for/list ([%i (reverse (range 0 m 1))])
        (define res
...
(define bv1024? (bitvector 1024))
(define bv512? (bitvector 512))
;;(bv1024? (mul_conv_32x8 xbuff_64_8 ybuff_64_8 32 8 8 32))
(bv512? (shift_bytes xbuff_32_16 ybuff_32_16 8 16 32))
(shift_bytes xbuff_32_16 ybuff_32_16 8 16 32)

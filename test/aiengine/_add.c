...
aie_bv *add16(aie_bv *a, aie_bv *b, uint8_t *offset_arr, int start, int num_elems) {
    uint8_t *idx = (uint8_t *)malloc(num_elems * sizeof(uint8_t));
...
int main() {
    ...
    uint8_t *offset_arr = (uint8_t *)malloc(num_elems * sizeof(uint8_t));

    int f;
    for (f = 0; f < num_elems; f++) {
        int byte = f >> 3;
        int n = sizeof(f) * 8 - 3;
        int offset = ((unsigned)f << n) >> n;
        if (f == 56) {
            offset_arr[byte] |= 1 << (7 - offset);
        } else {
            offset_arr[byte] &= ~(1 << (7 - offset));
        }
    }

    hex_out(offset_arr, 8);

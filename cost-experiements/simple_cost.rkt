...
(define (spec a b c) 
    (define dst (interpret a env))
    (define vec1 (interpret b env))
    (define vec2 (interpret c env))
    (apply  
      concat  
      (for/list ([j (range 4)])  
                (define tmp  
                  (bvmul (ext-bv vec1 (- 3 j) 32) (ext-bv vec2 (- 3 j) 32))) 
    ...
    (begin
      (assert
        (equal?
          ...
          (vector-mac x y z 4 32)
          ))
        (assert (<= (cost sketch) 25))
      )))

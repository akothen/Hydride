...
(define (grammar vars #:depth k)
  ...
  (cond
    ...
    [(choose* #t #f)
     (vec-load (grammar vars #:depth (- k 1)) 
               64 0 4 8
              )]
    [(choose* #t #f)
     (vec-load (grammar vars #:depth (- k 1)) 
    ...
    [(choose* #t #f)
     (vec-strided-gather (grammar vars #:depth (- k 1)) 
               192 0 6 4 8
              )]
    [(choose* #t #f)
     (vec-strided-gather (grammar vars #:depth (- k 1)) 
    ...
    [(choose* #t #f)
     (vec-reduction (grammar vars #:depth (- k 1)) 4 8)]
...
(define sketch (top-grammar (list (reg 0) (reg 1)) #:depth 4))
